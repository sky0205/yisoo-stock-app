...
st.set_page_config(
    page_title="이수할아버지의 냉정 진단기 v36117", layout="wide"
)
...
st.title("🧐 이수할아버지의 냉정 진단기 v36117 (호가창 실전 병법 장착)")
display_global_risk()
st.divider()
...
if symbol:
    try:
        ...
        if df.empty:
            ...
            # ==================================================================
            # ★★★ [v36117] 호가창 실전 판독 기준 완벽 적용 (오리지널 지침 반영)
            # ==================================================================
            ...
            if not is_kr and not has_manual_ob:
                ...
                if ob_ratio_val >= 1.5:
                    ob_status_msg = (
                        f"🔥 <b>[본진 투입 합격]</b> 매도/매수 잔량비 <b>{ob_ratio_val:.2f}배</b> "
                        f"(매도:{ask_formatted} / 매수:{bid_formatted}) - 매도벽을 걷어차는 강력한 매수세 확정 (3단계 합격)"
                    )
                    is_ob_stage2_safe = True
                    is_ob_stage3_safe = True
                elif ob_ratio_val >= 1.0:
                    ob_status_msg = (
                        f"⚖️ <b>[탈출 공방 호가]</b> 매도/매수 잔량비 <b>{ob_ratio_val:.2f}배</b> "
                        f"(매도:{ask_formatted} / 매수:{bid_formatted}) - 5일선 돌파 의지 확인 (2단계 합격 / 3단계 부족)"
                    )
                    is_ob_stage2_safe = True
                    is_ob_stage3_safe = False
                elif ob_ratio_val >= 0.5:
                    ob_status_msg = (
                        f"⚠️ <b>[속임수/윗꼬리 경계]</b> 매도/매수 잔량비 <b>{ob_ratio_val:.2f}배</b> "
                        f"(매도:{ask_formatted} / 매수:{bid_formatted}) - 1.0배 미만! 세력이 위로 살 의지가 없는 속임수 경계"
                    )
                    is_ob_stage2_safe = False
                    is_ob_stage3_safe = False
                else:
                    ob_status_msg = (
                        f"🚨 <b>[허매수 폭락 경고]</b> 매도/매수 잔량비 <b>{ob_ratio_val:.2f}배</b> "
                        f"(매도:{ask_formatted} / 매수:{bid_formatted}) - 매수잔량이 2배 이상 많아 바닥을 가장한 허매수 투매 위험!"
                    )
                    is_ob_stage2_safe = False
                    is_ob_stage3_safe = False
            elif manual_ask > 0 or manual_bid > 0:
            ...
            is_bottom_entry_signal = (
                ...
                # 1단계는 비율 제한은 없으나, 0.5 미만(허매수 폭락 경고)일 때는 차단
            ...
            if is_stop_loss_triggered:
                ...
                and pullback_rebound_score >= 1
                and (vol_strength >= 65 or is_pre_market_mode)
                and is_ob_stage3_safe # ★ 20일선 돌파도 3단계 본진과 같은 1.5배 필터 적용
            ):
                if margin_diff < 7.0:
                    final_code = "WAIT_NARROW_MARGIN"
            ...
            # ★ [v36117 추가] 지표는 맞으나 호가창 비율 미달 시 변명 금지 및 강제 보류
            elif (
                is_ma5_safe
                and not is_down_trend_structural
                and (bottom_score >= 1 or pullback_rebound_score >= 1)
                and has_manual_ob
                and ((pullback_rebound_score >= 1 and not is_ob_stage3_safe) or (bottom_score >= 1 and not is_ob_stage2_safe))
            ):
                final_code = "WAIT_ORDERBOOK"
                sig = "🟡 [관망/보류] 매도잔량비 실전 기준 미달 (허매수/속임수 경계)"
                col = "#F57C00"
                final_adv = (
                    f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                    f"<b>[호가창 기준 미달]</b> 타점 지표는 충족했으나, 매도/매수 잔량비({ob_ratio_val:.2f}배)가 실전 투입 기준에 미달하오. 세력의 윗꼬리 속임수이거나 바닥을 가장한 허매수일 확률이 높으니 방아쇠를 잠그고 관망하시게."
                )
            elif is_ma_tangled:
                if is_ma5_safe and vol_strength >= 65.0 and bandwidth >= 10.0 and (rsi_val <= 40 or temp_bottom_score >= 1 or (p_chg >= 0.0 and p >= mid_line)):
                    ...
                    final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). <b>[이평선 꼬임 속 진바닥 포착]</b> 수급과 밴드폭이 뒷받침된 상태에서 5일선 안착 및 냉골 바닥 지표가 충족되었으므로 비중 10% 분할 입절로 대응하시게."
                else:
            ...
            with i1:
                if is_long_upper_tail: bb_diag = "🟡 <b>[위꼬리 저항 관망 구역]</b><br>• <b>진단:</b> 긴 위꼬리가 밀려 내려왔으니 섣부른 추격을 금하고 관망."
                elif final_code == "WAIT_NARROW_MARGIN": bb_diag = f"🟡 <b>[상승 여력 부족 / 관망] (여력: +{margin_diff:.1f}%)</b><br>• <b>진단:</b> 타점 조건은 좋으나 상단 목표선까지 거리가 너무 좁소. 진입 보류."
                elif final_code == "WAIT_ORDERBOOK": bb_diag = f"🟡 <b>[호가창/매도벽 미달 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 매도잔량비 기준 미달! 세력의 위로 살 의지가 약하거나 허매수로 의심되니 진입 보류."
                elif final_code == "WAIT_INDICATOR":
                    if pullback_rebound_score >= 1: bb_diag = f"🟡 <b>[추세/안전마진 대기 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 점수는 충족했으나 안전마진 부족으로 관망."
                    else: bb_diag = f"🟡 <b>[지표/수급 대기 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 지표 동조 점수 부족({pullback_rebound_score}/3점)으로 안착 대기 중."
                    ...
                    if pullback_rebound_score >= 1: bb_diag = f"⚖️ <b>[관망 및 대기 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 지표 동조는 되었으나 역배열 등 조건 미달로 관망."
                    else: bb_diag = f"⚖️ <b>[관망 및 대기 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 지표 동조 점수 미흡({pullback_rebound_score}/3점)으로 안착 대기 중."
            ...
            with i4:
                if is_band_riding: m_diag = "<b>🔥 엔진 풀가동 (대세 추종)</b><br>• <b>진단:</b> 밴드 확장과 함께 엔진이 힘을 내고 있소! 50% 수확 후 5일선 사수."
                elif is_target_reached or p >= (target_price_100 * 0.99): m_diag = "<b>🚨 엔진 과열 차단</b><br>• <b>진단:</b> 목표선 도달 완료로 추가 가속 중단! 잔여 물량 익절에 집중하시게."
                elif is_overall_cautious_state and not (is_kr and is_morning_breakout_fast): m_diag = f"{base_macd_desc}<br>• <b>[관망 기조 동조]:</b> 상단 결론이 관망 상태이므로, 섣부른 추격매수를 금하고 관망."
                else: m_diag = f"{base_macd_desc}<br>• <b>[엔진 연동]:</b> 위 전황에 맞춰 유효하게 대응하시게."
