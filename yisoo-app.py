...
st.markdown("""
    ...
    .trend-card { font-size: 22px; line-height: 1.8; color: #000000 !important; padding: 25px; background: #F0F4F8; border-left: 12px solid #1E3A8A; border-radius: 12px; margin-bottom: 25px; }
...
st.title("👨‍💻 이수할아버지의 '무결점' 분석기 v13000")
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [출력 3] 상세 수치 (LargeUtf8 에러 방지를 위해 모든 데이터를 글자로 변환)
