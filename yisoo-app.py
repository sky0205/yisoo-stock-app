...
# 1. 화면 구성 및 할배 캐릭터 스타일 (실전용 박스 레이아웃)
...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    .vol-sub-text { font-size: 30px !important; color: #1565C0 !important; line-height: 1.7; background-color: #F5F5F5; padding: 18px; border-radius: 8px; border-left: 12px solid #1E88E5; }
    
    /* 지수 훈수 박스 상세 스타일 (내용 복구) */
    ...
    .ind-title { font-size: 38px !important; color: #1976D2 !important; border-bottom: 4px solid #EEEEEE; padding-bottom: 15px; margin-bottom: 20px; }
    ...
    /* 대왕 글씨 metric 수치 */
    [data-testid="stMetricValue"] { font-size: 60px !important; font-weight: 900 !important; }
    [data-testid="stMetricLabel"] { font-size: 30px !important; color: #546E7A !important; }
    </style>
    """, unsafe_allow_html=True)

# 글로벌 전황: 박스는 살리고 제목은 글씨로만
def display_global_risk():
    try:
        ...
        st.markdown("<div class='real-box'>", unsafe_allow_html=True)
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            st.markdown(f"<div style='background-color:{col}; padding:40px; border-radius:20px; text-align:center; margin-bottom:30px;'><p style='font-size:85px; font-weight:900; color:white; margin:0;'>{sig}</p></div>", unsafe_allow_html=True)

            # 성벽 가격선: 박스는 살리고 제목은 글씨로
