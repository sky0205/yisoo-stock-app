...
if symbol:
    try:
        ...
        if is_kr:
            ...
            # 1. 네이버 모바일 API로 정확한 실시간 현재가 및 한글 종목명 수집
            ...
            # 2. 기술적 지표 계산용 차트 수집 (양방향 탐색: .KS -> .KQ)
            try:
            ...
            # 만약 네이버에서 종목명을 못 가져왔다면 yfinance info에서 한글/영문 이름 추출 시도
            if not final_display_name:
                try:
                    info_tk = yf.Ticker(f"{code_str}.KS")
                    if not info_tk.history(period='1d').empty:
                        final_display_name = info_tk.info.get('longName') or info_tk.info.get('shortName') or f"국내종목 ({code_str})"
                    else:
