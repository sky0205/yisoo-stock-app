...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [2순위 핵심 수선] 주가는 올랐으나 거래량이 못 받쳐줄 때 호통 로직
            v_curr = df['Volume'].iloc[-1]; v_avg5 = df['Volume'].iloc[-6:-1].mean(); v_ratio = (v_curr / v_avg5) * 100 if v_avg5 > 0 else 0
            
            if chg_rate > 2.0 and v_ratio < 100: # 주가는 2% 넘게 뛰었는데 거래량은 평균 미달일 때
                v_status = "💤 거래침체 (가짜 상승 주의!)"
                v_advice = "🚨 <b>주가 급등했으나 거래량이 태부족!</b> 빈집에 바람 드는 격이니 절대 속지 마십시오. 세력들이 물량 넘기고 도망갈 준비 하는 것일 수도 있습니다."
            elif v_ratio < 100:
                v_status = "💤 거래침체"
                v_advice = "⏳ <b>눈치보기 중.</b> 파는 사람도 없으니 바닥 확인될 때까지 섣불리 물타지 마십시오."
            elif v_ratio >= 200:
                v_status = "🔥 거래폭발 (세력 진격!)"
                v_advice = "🚀 <b>큰손들이 문 부수고 들어왔습니다!</b> 장대양봉이면 기세 좋게 진격 수위를 높이셔도 좋구먼요."
            else:
                v_status = "📈 거래보통"
                v_advice = "✅ 세력의 발자국을 차분히 추적 중일세."

            st.markdown(f"<div class='vol-box'><div class='vol-main-text'>📊 거래량 전황: {v_ratio:.1f} % ({v_status})</div><div class='vol-sub-text'>{v_advice}</div></div>", unsafe_allow_html=True)

            # 지수 설명 (어르신 필살 문구 100% 복구)
            st.divider()
            ...
            with i1: st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>Bollinger (기세)</p><p class='ind-diag'>● **[상단 돌파!]** 하늘 찌르네! 익절 준비 하시게. 조만간 수익 실현 소나기가 올 수 있으니 보따리 싸시게.</p></div>", unsafe_allow_html=True)
            with i2: st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>RSI (온도)</p><p style='font-size:40px; color:#E65100;'>{rsi_val:.1f}</p><p class='ind-diag'>● 온도 {rsi_val:.2f}로 **탐욕의 불지옥** 직전입니다. 남들 환호할 때 냉정하게 분할 매도로 수익을 챙기시게.</p></div>", unsafe_allow_html=True)
            with i3:
                h14 = df['High'].rolling(14).max().iloc[-1]; l14 = df['Low'].rolling(14).min().iloc[-1]; will_val = (h14 - p) / (h14 - l14 + 1e-10) * -100
                st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>Williams %R</p><p style='font-size:40px; color:#E65100;'>{will_val:.1f}</p><p class='ind-diag'>● 지수 {will_val:.1f}로 **항복 지점** 근처입니다. 바닥권이니 이제 분할 매수 보따리를 푸시게.</p></div>", unsafe_allow_html=True)
            with i4: st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>MACD (엔진)</p><p class='ind-diag'>● {'엔진 정회전! 기세 좋구먼.' if m_l > s_l else '엔진 **역회전** 중! 절대 속지 마시게.'} 폭풍우 지나가길 기다리는 게 상책일세.</p></div>", unsafe_allow_html=True)
