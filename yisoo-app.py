...
# 1. 화면 구성 및 할배 캐릭터 스타일 (빈 박스 제거 및 가독성 극대화)
...
if symbol:
    try:
        ...
        if is_kr:
            df = fdr.DataReader(symbol, start_date, end_date); stocks = fdr.StockListing('KRX'); name = stocks[stocks['Code'] == symbol]['Name'].values[0]; currency = "원"; fmt_p = ",.0f"
        else:
            ticker = yf.Ticker(symbol); df = ticker.history(start=start_date, end=end_date); name = ticker.info.get('shortName', symbol); currency = "$"; fmt_p = ",.2f"
        ...
        if not df.empty:
            ...
            if v_ratio < 100: v_adv = f"🚨 **가짜 상승 주의!** 거래량 {v_ratio:.1f}%로 빈 수레일세. 속지 마시게." if p > prev_p else "⏳ **눈치보기.** 바닥 확인까지 물타지 마시게."
            elif v_ratio >= 200: v_adv = f"🚀 **세력 진격!** 거래량 {v_ratio:.1f}% 폭발! 기세 타시게." if p > prev_p else "💣 **투매 발생!** 비명 소리 나니 대피하시게."
            else: v_adv = f"✅ 세력의 발자국 추적 중. ({v_ratio:.1f}%)"
            st.markdown(f"<div class='vol-main-text'>{v_label} ({v_ratio:.1f}%)</div><div class='vol-sub-text'>{v_adv}</div>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
            ...
            # 4대 지수 상세 분석: 박스 안에 복구
            st.divider()
            i1, i2 = st.columns(2); i3, i4 = st.columns(2)
            with i1: # Bollinger
                bb_diag = "● **[비상: 상단 돌파!]** 하늘 찌르네! 익절 준비 하시게." if p >= up_b else "● **[비상: 하단 돌파!]** 바닥권일세! 기회 보시게." if p <= low_b else "● 성벽 사수 확인 전까지 대기!"
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p class='ind-diag'>{bb_diag}</p></div>", unsafe_allow_html=True)
            with i2: # RSI
                r_diag = f"● 지수 {rsi_val:.1f}로 **👺 불지옥** 문턱일세! 익절가 잡으시게." if rsi_val >= 65 else f"● 지수 {rsi_val:.1f}로 **🧊 냉골** 상태일세! 바닥 보시게." if rsi_val <= 35 else "● 탐욕과 공포 사이 중립일세."
            ...
            with i3: # Williams
                ...
                w_diag = f"● 지수 {will_val:.1f}로 **🏳️ 개미 항복** 구간! 보따리 푸시게." if will_val < -80 else f"● 지수 {will_val:.1f}로 **🧨 천장 광기**!" if will_val > -20 else "● 중간지대일세."
