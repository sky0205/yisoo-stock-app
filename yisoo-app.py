...
st.markdown(f"""
    <div class='global-unified-box'>
        ...
        <div style='font-size:16px; color:#CFD8DC; line-height:1.6;'>
            <b>👴 냉정 평가:</b> 미장 지수는 거품 위 줄타기 중이고 국채 금리는 언제든 비수가 될 수 있네. 국장은 사지가 마비된 형국이니 절대 섣불리 진격하지 마시게.
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            if p > prev_p: v_advice = f"📈 <b>주포 진격:</b> {pct_chg:.1f}% 급등에 거래량까지 실렸네." if v_ratio > 150 else "🚨 <b>가짜 축제:</b> 거래량 없이 올린 함정일세."
            ...
            # 2. 필살 전략 + 냉정 진단 (실시간 등락 완벽 반영)
            is_surge = pct_chg >= 5.0; is_new_high = p >= peak_p * 0.98
            if is_new_high:
                diag_txt = f"성벽 상단을 부수고 {pct_chg:.1f}% 폭등하며 신고가 영역일세. 주포가 작정하고 문을 연 것이야."
                strategy_txt = "<b>수익 극대화:</b> 상단선 무시하고 기세를 즐기셔. 전일 고점을 생명선 삼고, 새로 타는 건 독약일세."
            elif is_surge:
                diag_txt = f"오늘 {pct_chg:.1f}% 급등하며 하락 추세를 단번에 돌파했네. 장부가 생기를 되찾았구먼."
            ...
            # 4. 네 기둥 지수 상세 진단 (종목 맞춤형 훈수 로직 고정)
            ...
            with i4: # MACD
                if m_l > 0 and m_l > s_l: m_msg = f"🚀 엔진 {m_l:.2f}, 가속 중."
                elif m_l > 0 and m_l <= s_l: m_msg = f"⚠️ 엔진 {m_l:.2f}, 꺾였어."
                elif m_l <= 0 and m_l > s_l: m_msg = f"📈 엔진 {m_l:.2f}, 고개 드네."
                else: m_msg = f"💣 엔진 {m_l:.2f}, 역회전 중."
