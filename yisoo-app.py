...
target_symbol = st.text_input("📊 종목코드(6자리) 또는 티커 입력", value="IONQ").strip().upper()
...
if target_symbol:
    try:
        ...
        if not df.empty:
            ...
            is_us = not target_symbol.isdigit() # 미국 주식 여부 판별
            
            # --- [지수 계산: BB(20,2), RSI(14,6), WR(14,9)] ---
            ...
            rsi_v = float(rsi_m.iloc[-1]); rsi_s = float(rsi_m.rolling(6).mean().iloc[-1])
            
            h14 = df['high'].rolling(14).max(); l14 = df['low'].rolling(14).min()
            wr_m = ((h14 - df['close']) / (h14 - l14)) * -100
            wr_v = float(wr_m.iloc[-1]); wr_s = float(wr_m.rolling(9).mean().iloc[-1])
            ...
            <div class='analysis-card'>
                <b>② 수급 지수 (Will %R 14, 9):</b> {wr_v:.1f} 수치로 볼 때 {'단기 자금이 유입' if wr_v > wr_s else '단기 자금이 이탈'}되는 국면입니다.
            </div>
            <div class='analysis-card'>
                <b>③ 변동성 (BB 20, 2):</b> {lo_b:,.1f} ~ {up_b:,.1f} 사이에서 {'하단 지지력' if curr_p < ma20.iloc[-1] else '상단 저항력'}을 테스트 중입니다.
            </div>
            """, unsafe_allow_html=True)
