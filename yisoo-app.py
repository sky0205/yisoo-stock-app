...
if target_symbol:
    try:
        ...
        if not df.empty:
            ...
            # --- [선생님 전용 지수 계산: BB(20,2), RSI(14,6), WR(14,9)] ---
            # 볼린저 밴드
            ...
            rsi_m = 100 - (100 / (1 + (gain / loss)))
            rsi_v = float(rsi_m.iloc[-1])
            rsi_s = float(rsi_m.rolling(6).mean().iloc[-1])
            
            # Williams %R 및 시그널
            h14 = df['high'].rolling(14).max()
            l14 = df['low'].rolling(14).min()
            wr_m = ((h14 - df['close']) / (h14 - l14)) * -100
            wr_v = float(wr_m.iloc[-1])
            wr_s = float(wr_m.rolling(9).mean().iloc[-1])
            
            # MACD
            exp12 = df['close'].ewm(span=12, adjust=False).mean()
            exp26 = df['close'].ewm(span=26, adjust=False).mean()
            macd_v = float((exp12 - exp26).iloc[-1])
            macd_s = float((exp12 - exp26).ewm(span=9, adjust=False).mean().iloc[-1])
            ...
            p_val = f"${curr_p:,.2f}" if not target_symbol.isdigit() else f"{curr_p:,.0f}원"
            ...
            <div class='analysis-card'>
                <b>③ 변동성 (BB 20, 2):</b> 밴드 범위 {lo_b:,.0f} ~ {up_b:,.0f} 사이에서 
            ...
            with col1:
                st.markdown(f"<div class='detail-card'>볼린저 (20,2,c)<br>{lo_b:,.0f} ~ {up_b:,.0f}</div>", unsafe_allow_html=True)
