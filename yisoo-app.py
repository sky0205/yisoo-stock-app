...
if ai_data:
    ...
    if st.button("🚀 실시간 4대 지표 통합 분석 시작"):
        ...
        if not df.empty:
            # [ValueError 해결] 모든 지표를 스칼라(Scalar) 숫자로 강제 변환
            close = df['Close']
            ...
            st.table(pd.DataFrame({
                ...
                "진단": ["과열 주의" if price > up_band else "바닥 기회" if price < dn_band else "정상", "심리 과열" if rsi>70 else "심리 바닥" if rsi<30 else "보통", "단기천장" if wr>-20 else "단기바닥" if wr<-80 else "보통", "상승세 우세" if macd_val>0 else "하락 압력"]
            }))
        else:
