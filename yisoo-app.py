...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [상세 보완] 4대 지수 훈수 (image_abbc54.png 기반 100% 복구)
            st.divider()
            i1, i2, i3, i4 = st.columns(4)
            with i1: # Bollinger
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p class='ind-diag'>● **[상단 돌파!]** 하늘 찌르네! 익절 준비 하시게. 평균 회귀 본능에 의해 조만간 수익 실현 소나기가 올 수 있으니 보따리 싸시게.</p></div>", unsafe_allow_html=True)
            with i2: # RSI
                r_stat = "👺 불지옥" if rsi_val > 65 else "🧊 냉골" if rsi_val < 35 else "미지근"
                r_diag = f"● 온도 {rsi_val:.2f}로 **탐욕의 불지옥** 직전입니다. 남들이 환호할 때 냉정하게 분할 매도로 수익을 확정 지으십시오." if r_stat=="👺 불지옥" else f"● 온도 {rsi_val:.2f}의 **냉골 상태**입니다. 지수가 완전히 식어버릴 때까지 낚싯대만 던져두고 기다리십시오." if r_stat=="🧊 냉골" else "● 매수와 매도가 팽팽한 줄다리기 중일세."
            ...
            with i3: # Williams
                ...
                w_diag = f"● **불나방 광기(-20↑)** 직전! 지수 {will_val:.2f}는 천장 근처이니 추격하지 말고 후퇴를 준비하십시오." if will_val > -35 else f"● **항복 지점(-75↓)** 근처! 지수 {will_val:.2f}로 바닥권이니 이제 분할 매수 보따리를 푸십시오." if will_val < -65 else "● 안개 속이니 방향이 정해질 때까지 자중하며 째려만 보시게."
            ...
            with i4: # MACD
                st.markdown(f"<div class='ind-box'><p class='ind-title'>MACD (엔진)</p><p class='ind-diag'>● {'엔진 정회전! 기세 좋구먼.' if m_l > s_l else '엔진 **역회전** 중! 절대 속지 마시게.'} 섣부른 진격보다는 정박하여 폭풍우가 지나가길 기다리는 게 상책일세.</p></div>", unsafe_allow_html=True)
