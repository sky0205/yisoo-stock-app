...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    /* 4. 네 기둥 지수 상세 진단 */
    .ind-box { background-color: #FFFFFF; padding: 15px; border-radius: 10px; border: 2px solid #90A4AE; min-height: 320px; margin-bottom: 10px; }
    ...
    .ind-diag { font-size: 15px !important; line-height: 1.5; }
...
st.markdown(f"""
    <div class='global-unified-box'>
        <div style='font-size:18px; color:#81D4FA; margin-bottom:10px;'>🌎 0. 글로벌 전황 보고</div>
        ...
        <div style='font-size:14px; color:#CFD8DC; margin-top:10px;'><b>👴 냉정 평가:</b> 미장 거품 위태롭고 금리는 비수가 될 수 있네. 국장은 사지 묶인 격이야.</div>
    </div>
    """, unsafe_allow_html=True)
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            st.markdown(f"""
                <div class='unified-strategy-box'>
                    ...
                    <div class='diagnosis-content'>
                        <b>⚠️ [냉정 진단]:</b> 주가가 중앙선({format(mid_line, fmt)}) 근처일세. <b>'데드 캣 바운스'</b>인지 아닌지는 지표가 말해주네. RSI <b>{rsi_val:.1f}</b>는 아직 거품이 덜 빠졌음을 뜻해.
                    </div>
                    <div style='font-size:16px; color:#333;'>● <b>지침:</b> 속지 마시게. 지표가 바닥 심해로 잠수할 때까지 독하게 기다려야 하네.</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            # 4. 네 기둥 지수 상세 진단 (지표 제목 부착)
            st.subheader("🏗️ 4. 네 기둥 지수 상세 진단")
            i1, i2, i3, i4 = st.columns(4)
            with i1: # Bollinger
                bb_txt = "⚠️ 하단 성벽 돌파! 칼날 잡지 마셔." if p <= low_b else "⚖️ 중앙선 아래서 빌빌대네." if p < mid_line else "📈 중앙선 위에서 기세 타네."
                st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ Bollinger</p><span style='font-size:14px; color:#546E7A;'>[중앙선 가격]</span><span class='ind-value'>{format(mid_line, fmt)}</span><p class='ind-diag'>● {bb_txt}</p></div>", unsafe_allow_html=True)
            with i2: # RSI
                st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ RSI</p><span style='font-size:14px; color:#546E7A;'>[현재 지수]</span><span class='ind-value'>{rsi_val:.1f}</span><p class='ind-diag'>● 강도 <b>{rsi_val:.1f}</b>일세. 30 이하로 식을 때까지 총 아끼게.</p></div>", unsafe_allow_html=True)
            with i3: # Williams
                st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ Williams</p><span style='font-size:14px; color:#546E7A;'>[현재 지수]</span><span class='ind-value'>{will_val:.1f}</span><p class='ind-diag'>● 수치 <b>{will_val:.1f}</b>일세. -80 아래 심해가 진짜 바닥일세.</p></div>", unsafe_allow_html=True)
            with i4: # MACD
                st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ MACD</p><span style='font-size:14px; color:#546E7A;'>[엔진 상태]</span><span class='ind-value' style='font-size:28px !important;'>{'▲ 상승' if m_l > s_l else '▼ 하락'}</span><p class='ind-diag'>● 엔진 {'정방향' if m_l > s_l else '역회전'} 중일세. 맥점 째려보셔.</p></div>", unsafe_allow_html=True)

    except Exception as e: st.error(f"장부 오류: {e}")

st.write("---")
st.caption("분석가 서강윤: 2026년 실시간 장부 및 v36056 모바일 최종본")
