...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            trend_desc = "정배열 상승 가도" if (m_l > s_l and p > df['MA20'].iloc[-1]) else "역배열 하락 늪" if (m_l < s_l and p < df['MA20'].iloc[-1]) else "횡보 및 안개 정국"
            ...
            with i3: # Williams %R (어르신 지적 반영 교정)
