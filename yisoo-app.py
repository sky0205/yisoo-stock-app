...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # --- 이수 할배의 5단계 실전 신호등 (보유/비보유 분리) ---
            if p < defense_line or (m_l < s_l and p < mid_line):
                sig, col = "💀 퇴각 (매도 2단계)", "#263238"
                s_adv = "● [보유] 미련 없이 전원 퇴각(전량 매도)! / [비보유] 성문 굳게 닫고 절대 금지!"
                final_adv = f"💀 **[최종 결론] 퇴각**. 거래강도({v_score:.0f}점). 성벽 함락! 살고 싶으면 당장 도망치시게."

            elif rsi_val >= 70 or p >= up_b:
                sig, col = "💰 수확 (매도 1단계)", "#388E3C"
                s_adv = "● [보유] 수익금 챙겨 소량 익절! / [비보유] 탐욕에 눈멀어 진입하면 지옥행일세!"
                final_adv = f"💰 **[최종 결론] 수확**. 거래강도({v_score:.0f}점). 과열권이니 분할 매도로 주머니 채우시게."

            elif m_l > s_l and p >= defense_line and v_score > 100:
                sig, col = "🔥 진격 (매수 2단계)", "#1E88E5"
                s_adv = "● [보유] 기세 타서 홀딩! / [비보유] 본진 투입(전량 매수)하여 세력과 함께 진격!"
                final_adv = f"🔥 **[최종 결론] 진격**. 거래강도({v_score:.0f}점). 엔진 정회전에 성벽 사수! 진격의 발판일세."

            elif (rsi_val <= 35 or will_val <= -80) and p <= low_b:
                sig, col = "☘️ 매복 (매수 1단계)", "#D32F2F"
                s_adv = "● [보유] 고통의 끝자락이니 버티시게! / [비보유] 정찰대(20~30%) 먼저 조용히 매복시키게."
                final_adv = f"☘️ **[최종 결론] 매복**. 거래강도({v_score:.0f}점). 바닥 신호니 조용히 보따리 풀 준비하시게."

            else:
                sig, col = "🧐 지옥행 (관망)", "#FBC02D"
                s_adv = "● [보유] 탈출 기회 엿보며 자숙! / [비보유] 강 건너 불구경하듯 절대 칼 뽑지 마시게."
                final_adv = f"🧐 **[최종 결론] 지옥행**. 거래강도({v_score:.0f}점). 추세 꺾였으니 냉정하게 관망하시게."
            ...
            if p >= up_b or rsi_val >= 60:
                if m_l < s_l:  # [꼭대기 3] 엔진 역회전 + 성벽 위태
                    if p < defense_line or abs(m_diff) > abs(m_diff_p):
                        final_adv = f"🚨 **[최종 결론]** 거래강도({v_score:.0f}점). 성벽 위태롭고 엔진 역회전 심화! **전량 익절**하시게!"
                    else:
                        final_adv = f"⚠️ **[최종 결론]** 거래강도({v_score:.0f}점). 성벽 사수 중이나 엔진 역회전 초입일세. **부분 익절**하시게."
                elif v_score > 150 and p > defense_line:  # [꼭대기 1] 비상
                    final_adv = f"🚀 **[최종 결론]** 거래강도({v_score:.0f}점). 성벽 딛고 하늘 문이 열렸네! **빳빳하게 홀딩**하시게!"
                else:  # [꼭대기 2] 정체
                    final_adv = f"💰 **[최종 결론]** 거래강도({v_score:.0f}점). 성벽 위나 기세가 약해지네. **분할 매도**로 수확하시게."

            # --- [바닥권 3대 전술: 성벽 탈환 시도] ---
            elif p <= (low_b * 1.02):
                if m_l < s_l or p < (defense_line * 0.90):  # [바닥 3] 금지
                    final_adv = f"💀 **[최종 결론]** 거래강도({v_score:.0f}점). 성벽 멀고 엔진 역회전이네. **절대 매수 금지**일세."
                elif v_score > 130 and p >= (defense_line * 0.95):  # [바닥 1] 진격
                    final_adv = f"🔥 **[최종 결론]** 거래강도({v_score:.0f}점). 진짜 바닥에 물량 실렸네! 적극 진격하시게!"
                else:  # [바닥 2] 정찰
                    final_adv = f"🛡️ **[최종 결론]** 거래강도({v_score:.0f}점). 엔진은 도는데 성벽이 멀구먼. **정찰대**만 보내시게."
