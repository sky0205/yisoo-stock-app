...
# 1. 화면 구성 및 할배 캐릭터 스타일 (일체형 박스 레이아웃)
...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    /* [핵심] 제목 지붕과 내용을 하나로 가두는 통합 섹션 박스 */
    .unified-box {
    ...
    .vol-sub-text { font-size: 28px !important; color: #1565C0 !important; line-height: 1.7; background-color: #F9F9F9; padding: 22px; border-radius: 10px; border-left: 12px solid #1E88E5; }
    
    /* 지수 훈수 박스 */
    .ind-box { background-color: #FFFFFF; padding: 30px; border-radius: 20px; border: 5px solid #90A4AE; min-height: 550px; margin-bottom: 25px; }
...
def display_global_risk():
    try:
        ...
        advice = "✅ **[진격!]** 미장 쾌청!" if n_chg > 0.5 else "🚨 **[정박!]** 피신하시게!" if n_chg < -1.0 else "🧐 **[관망]** 지표 끝단을 기다리시게."
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # 거래량 분석: 제목 지붕 아래 완벽 봉인
            st.markdown(f"""<div class='unified-box'>
                <div class='unified-header'>📊 실시간 거래량 전황 분석</div>
                <div class='unified-content'>""", unsafe_allow_html=True)
            v_label = "💤 거래침체" if v_ratio < 100 else "📈 거래증가"
            if v_ratio >= 30 and is_opening:
                v_adv = f"🔥 **[세력 진격!]** 거래량이 빳빳하게 터지며 폭등 중일세!" if p_chg >= 3 else f"💀 **[비명 포착!]** 거래량이 터지며 폭락 중이니 피신하시게!"
                v_status = f"🔥 시초 거래폭발 ({v_ratio:.1f}%)"
            else:
                v_status = f"{v_label} ({v_ratio:.1f}%)"
                v_adv = f"🚨 **[가짜 상승 주의!]** 거래량은 빈 수레인데 주가만 올랐네! 속지 마시게." if p_chg > 3 and v_ratio < 100 else f"✅ 세력의 발자국을 추적 중일세."
            ...
            st.markdown(f"<div class='signal-box' style='background-color:{col};'><p style='font-size:90px; font-weight:900; color:white; margin:0;'>{sig}</p><p style='color:white; font-size:40px;'>{s_adv}</p></div>", unsafe_allow_html=True)

            # 성벽 가격선: 제목 지붕 아래 완벽 봉인
            st.markdown(f"""<div class='unified-box'>
            ...
            st.markdown(f"""<div class='trend-card'><div class='trend-title'>⚔️ {name} 실전 필살 대응 전략</div>
                ...
                <div class='trend-item'>2. **성벽 사수:** 성벽({format(defense_line, fmt_p)}) {'함락!' if p < defense_line else '사수 중.'}</div>
                <div class='trend-item'>3. **엔진 확인:** {'정회전!' if m_l > s_l else '**역회전!** 속지 마시게.'}</div>
            ...
            with i2: # RSI
                r_diag = f"● 지수 {rsi_val:.2f}로 **👺 불지옥** 문턱일세! 익절가 잡으시게." if rsi_val >= 60 else f"● 지수 {rsi_val:.2f}로 **🧊 냉골** 상태일세! 바닥 보시게."
                st.markdown(f"<div class='ind-box'><p class='ind-title'>RSI (온도)</p><p style='font-size:70px; color:#E65100;'>{rsi_val:.1f}</p><p class='ind-diag'>{r_diag}</p></div>", unsafe_allow_html=True)
            with i3: # Williams
                w_diag = f"● 지수 {will_val:.2f}로 **🏳️ 개미 항복** 구간! 보따리 푸시게." if will_val < -80 else f"● 지수 {will_val:.2f}로 **🧨 천장 광기** 구간!"
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Williams %R</p><p style='font-size:70px; color:#E65100;'>{will_val:.1f}</p><p class='ind-diag'>{w_diag}</p></div>", unsafe_allow_html=True)
            with i4: # MACD
