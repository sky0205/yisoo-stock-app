...
# 글로벌 지수 판독 (image_abc7b5.png 기반 상세 문구 복구)
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [수정 3] 4대 지수 상세 설명 완벽 복구 (image_abbc54.png 기반)
            st.divider()
            i1, i2, i3, i4 = st.columns(4)
            with i1: # Bollinger
                st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>Bollinger (기세)</p><p class='ind-diag'>● **[상단 돌파!]** 하늘 찌르네! 익절 준비 하시게. 평균 회귀 본능에 의해 조만간 소나기가 올 수 있으니 보따리 싸시게.</p></div>", unsafe_allow_html=True)
            with i2: # RSI
                r_stat = "👺 불지옥" if rsi_val > 65 else "🧊 냉골" if rsi_val < 35 else "미지근"
                r_diag = f"● 온도 {rsi_val:.2f}로 **탐욕의 불지옥** 직전입니다. 남들이 환호할 때 냉정하게 분할 매도로 수익을 확정 지으십시오." if r_stat=="👺 불지옥" else f"● 온도 {rsi_val:.2f}의 **냉골 상태**입니다. 지수가 완전히 식어버릴 때까지 기다리십시오." if r_stat=="🧊 냉골" else "● 매수와 매도가 팽팽한 줄다리기 중일세."
            ...
            with i3: # Williams
                ...
                w_diag = f"● **불나방 광기(-20↑)** 직전! 후퇴를 준비하십시오." if will_val > -35 else f"● **항복 지점(-75↓)** 근처! 이제 분할 매수 보따리를 푸십시오." if will_val < -65 else "● 안개 속이니 째려만 보시게."
            ...
            with i4: # MACD
                st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>MACD (엔진)</p><p class='ind-diag'>● {'엔진 정회전! 기세 좋구먼.' if m_l > s_l else '엔진 **역회전** 중! 절대 속지 마시게.'} 폭풍우가 지나가길 기다리는 게 상책일세.</p></div>", unsafe_allow_html=True)
