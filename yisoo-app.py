...
# 1. 화면 구성 (어르신 원본 스타일을 유지하되, 빈 공간만 싹 제거)
...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    /* [수정] 텅 빈 박스 공간이 생기지 않도록 여백 및 테두리 정밀 조정 */
    .title-text { 
...
# 글로벌 전황: 빈 박스 없이 제목과 수치 박스만 딱 붙여 배치
def display_global_risk():
    try:
        ...
        # 꼴 보기 싫던 빈 박스 공간은 아예 지우고 알맹이 박스로 시작
        st.markdown("<div class='real-info-box'>", unsafe_allow_html=True)
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            st.markdown(f"<div class='signal-box' style='background-color: {col};'><span style='font-size:65px; font-weight:900; color:white;'>{sig}</span></div>", unsafe_allow_html=True)

            # 성벽 가격선 (빈 공간 제거)
            st.markdown("<span class='title-text'>🛡️ 매수·매도 핵심 성벽 가격선</span>", unsafe_allow_html=True)
            st.markdown("<div class='real-info-box'>", unsafe_allow_html=True)
            c1, c2, c3 = st.columns(3)
            with c1: st.markdown(f"<div class='price-card'><p>⚖️ 공략 대기선</p><p style='font-size:32px; color:#388E3C;'>{format(low_b, fmt)}</p></div>", unsafe_allow_html=True)
            with c2: st.markdown(f"<div class='price-card'><p>🎯 수확 목표선</p><p style='font-size:32px; color:#D32F2F;'>{format(up_b, fmt)}</p></div>", unsafe_allow_html=True)
            with c3: st.markdown(f"<div class='price-card'><p>🛡️ 성벽 (방어선)</p><p style='font-size:32px; color:#E65100;'>{format(defense_line, fmt)}</p></div>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
            ...
            with i2: # RSI
                r_diag = f"● 지수 {rsi_val:.1f}로 **👺 불지옥** 문턱일세! 익절가 잡으시게." if rsi_val >= 65 else f"● 지수 {rsi_val:.1f}로 **🧊 냉골** 상태일세! 바닥 보시게."
                st.markdown(f"<div class='ind-box'><p class='ind-title'>RSI (온도)</p><p style='font-size:40px; color:#E65100;'>{rsi_val:.1f}</p><p class='ind-diag'>{r_diag}</p></div>", unsafe_allow_html=True)
            with i3: # Williams
                h14 = df['High'].rolling(14).max().iloc[-1]; l14 = df['Low'].rolling(14).min().iloc[-1]; will_val = (h14 - p) / (h14 - l14 + 1e-10) * -100
                w_diag = f"● 지수 {will_val:.1f}로 **🏳️ 개미 항복** 구간! 보따리 푸시게." if will_val < -80 else f"● 지수 {will_val:.1f}로 **🧨 천장 광기**!"
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Williams %R</p><p style='font-size:40px; color:#E65100;'>{will_val:.1f}</p><p class='ind-diag'>{w_diag}</p></div>", unsafe_allow_html=True)
            with i4: # MACD
