...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    .unified-box {
        ...
        margin-bottom: 30px;
        overflow: hidden; /* 제목 배경이 박스 테두리에 딱 맞게 */
        padding: 0 !important;
    }
    .unified-header {
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            st.markdown(f"<div class='signal-box' style='background-color:{col};'><p class='signal-text'>{sig}</p><p style='color:white; font-size:40px;'>{s_adv}</p></div>", unsafe_allow_html=True)
            ...
            with i2: # RSI
                r_diag = f"● 지수 {rsi_val:.2f}로 **👺 불지옥** 문턱일세! 탐욕 버리고 익절가 잡으시게." if rsi_val >= 60 else f"● 지수 {rsi_val:.2f}로 **🧊 냉골** 상태일세! 냉정하게 바닥을 보시게." if rsi_val <= 35 else f"● 탐욕과 공포 사이 중립 기어일세."
