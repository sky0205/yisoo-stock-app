...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            with i1: # Bollinger
                bb_diag = f"● **[비상: 상단 돌파!]** 탐욕 버리고 수확(익절) 준비 하시게." if p >= up_b else f"● **[비상: 하단 돌파!]** 바닥권일세! 진격 기회일세." if p <= low_b else f"● 성벽 사수 확인 전까지는 기다리시게."
            ...
            with i4: # MACD
                m_diag = "● 엔진이 정회전 중!" if m_l > s_l else f"● 엔진이 **역회전** 중이네! 절대 속지 마시게."
