    ...
    <div class='global-unified-box'>
        ...
        <div class='global-item-container'>
            <div class='global-item'>
                ...
                <span class='global-item-val' style='color:{"#EF5350" if n_p > 0 else "#42A5F5"};'>{n_v} ({n_p:+.2f}%)</span>
            </div>
            <div class='global-item'>
                <span class='global-item-label'>S&P 500 (지수)</span>
                <span class='global-item-val' style='color:{"#EF5350" if s_p > 0 else "#42A5F5"};'>{s_v} ({s_p:+.2f}%)</span>
            </div>
            <div class='global-item'>
                <span class='global-item-label'>미국채 10년 금리</span>
                <span class='global-item-val' style='color:{"#EF5350" if t_p > 0 else "#42A5F5"};'>{t_v} ({t_p:+.2f}%)</span>
            </div>
        </div>
            ...
            st.markdown(f"""
                <div class='unified-strategy-box'>
                    ...
                    <div class='diagnosis-content'>
                        <b>⚠️ [냉정 진단]:</b> 현재 주가는 Bollinger Band 중앙선({format(mid_line, fmt)}){'을 한참 하회하여 하단 성벽으로 끌려가는 중일세.' if p < mid_line * 0.97 else ' 위에서 알짱거리지만 데드 캣 바운스일 뿐이야.' if p > mid_line * 1.03 else ' 근처에서 눈치만 보며 미지적대고 있구먼.'} RSI <b>{rsi_val:.1f}</b>는 거품이 여전함을 뜻해.
                    </div>
...
st.write("---")
st.caption("분석가 서강윤: 2026년 실시간 장부 및 v36056 정밀 보완본")
