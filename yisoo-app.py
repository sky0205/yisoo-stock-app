...
st.title("👨‍💻 이수할아버지의 '완전 복구' 분석기 v3000")

# [기능 1] 실시간 환율 및 종목명 캐싱
...
usd_krw, krx_list = get_global_data()

# [기능 2] 종목명 확인 함수
def find_name(symbol, krx_df):
    if not symbol.isdigit(): return symbol # 미장은 코드 그대로
    if not krx_df.empty:
        match = krx_df[krx_df['Code'] == symbol]
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            stock_name = find_name(symbol, krx_list)
            ...
            index_summary = pd.DataFrame({
                ...
                "현재 수치": ["상승" if macd.iloc[-1] > signal.iloc[-1] else "하락", f"{wr:.2f}", f"{curr_p:,.0f}"],
                "상태 진단": ["매수 우위" if macd.iloc[-1] > signal.iloc[-1] else "추세 확인 중", "과매도(바닥)" if wr < -80 else "정상", "안전 마진 확보" if curr_p < lo_b.iloc[-1] else "밴드 내 위치"]
            })
