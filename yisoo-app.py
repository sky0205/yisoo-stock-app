...
# 1. 화면 구성 (v36056 스타일 및 할배 캐릭터 완벽 복구)
...
def display_global_risk():
    ...
    except: st.error("⚠️ 글로벌 데이터 판독 불가")

# 제목 (캐릭터: 안경 쓴 회백색 젠틀 할아버지)
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # 오늘 제외한 과거 1년 최고가(예: 흥아해운 3,290원)
            past_high = float(df.iloc[:-1]['Close'].max()) 
            ...
            # [부활] 진짜 전고점을 넘었을 때만 역사적 신고가 인정
            if p > past_high: sig, col, adv = "🚀 역사적 신고가", "#D32F2F", f"● 1년 최고가({format(past_high, fmt_p)})를 넘었구먼! 즐기되 익절가 잡으시게."
            elif p >= past_high * 0.95: sig, col, adv = "⚔️ 성벽 돌파중", "#E65100", f"● 신고가는 무슨! 전고점({format(past_high, fmt_p)}) 성벽이 안 보이나?"
            elif p <= low_b or rsi_val <= 35: sig, col, adv = "🔴 매수권 진입", "#D32F2F", "● 바닥권일세. 분할 매수 보따리 푸시게."
            else: sig, col, adv = "🟡 관망 및 대기", "#FBC02D", f"● 아직 전고점({format(past_high, fmt_p)}) 밑일세. 헛꿈 꾸지 마시게."
            ...
            st.markdown(f"""<div class='trend-card'><div class='trend-title'>⚔️ {name} 실전 필살 대응 전략</div>
                <div class='trend-item'>● <b>추세 진단:</b> {"상승 가도" if p > mid_line else "하락 늪"} 상태일세. 중앙선({format(mid_line, fmt_p)}) 기준 판독하시게.</div>
                <div class='trend-item'>● <b>수비 상태:</b> 성벽({format(defense_line, fmt_p)}) {'함락!' if p < defense_line else '사수 중.'}</div>
                <div class='trend-item'>● <b>필살 조언:</b> <span class='advice-highlight'>{'전고점 돌파 여부를 눈을 부라리고 보시게!' if p >= past_high * 0.93 else '바닥을 더 확인하십시오.'}</span></div></div>""", unsafe_allow_html=True)

            # [현실적 훈수 복구] 네 기둥 지수 상세 진단
            i1, i2, i3, i4 = st.columns(4)
            with i1: st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p class='ind-status'>{'📈 상승' if p > mid_line else '📉 하락'}</p><p class='ind-diag'>현재 중앙선({format(mid_line, fmt_p)}) {'위' if p > mid_line else '아래'}일세. {'기세 타는 중이니 보시게!' if p > mid_line else '빌빌대니 성벽 무너지면 보따리 싸시게!'}</p></div>", unsafe_allow_html=True)
            with i2: st.markdown(f"<div class='ind-box'><p class='ind-title'>RSI (온도)</p><p style='font-size:40px; color:#E65100;'>{rsi_val:.2f}</p><p class='ind-diag'>● **{'👺 불지옥' if rsi_val > 65 else '🧊 냉골' if rsi_val < 35 else '미지근'}** 상태일세. 남들 환호할 때 냉정하게 보시게.</p></div>", unsafe_allow_html=True)
            with i3: # Williams 상세 훈수
                w_status = "🧨 천장광기" if will_val > -25 else "🏳️ 바닥항복" if will_val < -75 else "중간지대"
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Williams %R</p><p style='font-size:40px; color:#E65100;'>{will_val:.2f}</p><p class='ind-diag'>● **{w_status}** 구간일세. 천장 끝단인지 바닥인지 매섭게 보시게.</p></div>", unsafe_allow_html=True)
