...
if symbol:
    try:
        # [핵심 수선] 오늘을 제외한 순수 과거 데이터로만 최고가 판독
        ...
        if not df.empty:
            ...
            # [수선] 오늘 데이터를 완전히 도려낸 과거 최고가 (흥아해운 3,290원 타겟)
            ...
            # [수선] 진짜 전고점을 넘었을 때만 역사적 신고가 인정 및 노란불 유지
            if p > high_365:
                sig, col, adv = "🚀 역사적 신고가", "#D32F2F", f"● 진짜 1년 최고가({format(high_365, fmt_p)})를 드디어 넘었구먼! 즐기되 익절가 잡으시게."
            ...
            # [복구] 네 기둥 지수 상세 진단 (현실 밀착형 훈수)
            i1, i2, i3, i4 = st.columns(4)
            with i1: # Bollinger
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p class='ind-status'>{'📈 상승' if p > mid_line else '📉 하락'}</p><p class='ind-diag'>현재 중앙선({format(mid_line, fmt_p)}) {'위' if p > mid_line else '아래'}일세. {'기세 타는 중이니 보시게!' if p > mid_line else '빌빌대니 성벽 무너지면 보따리 싸시게!'}</p></div>", unsafe_allow_html=True)
            with i2: # RSI
