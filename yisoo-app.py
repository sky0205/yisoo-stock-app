...
st.set_page_config(
    page_title="이수할아버지의 냉정 진단기 v36106", layout="wide"
)
...
st.title("🧐 이수할아버지의 냉정 진단기 v36106 (MACD 출력 오류 완벽 보수)")
display_global_risk()
st.divider()
    ...
    try:
        ...
        if df.empty:
            ...
            if bandwidth < 10.0:
                ...
                bw_diag_msg = (
                    f"밴드폭 극소({bandwidth:.1f}%) 에너지 응축 중 / 돌파 방향 확인 대기"
                )
                squeeze_info_str = (
                    f"<br>• ⚡ <b>[밴드폭 극소({bandwidth:.1f}%)]</b> 에너지가 바짝 응축 중이오! 돌파 방향 확인 전까지 진입 금지."
                )
                ...
                if p >= ma5_val:
                    ...
                    if is_bearish_zone:
                        ...
                        squeeze_info_str = (
                            f"<br>• ⚠️ <b>[밴드폭 응축({bandwidth:.1f}%)]</b> 5일선 위 안착 중이나, 머리 위 역배열 저항 매물벽을 경계하시오."
                        )
                    else:
                        is_bandwidth_ok = True
                        bw_status_category = "SQUEEZE_BREAKOUT"
                        bw_diag_msg = (
                            f"밴드폭 응축돌파({bandwidth:.1f}%) 5일선 안착 / 에너지 분출 초입"
                        )
                        if vol_strength < 65 or is_candle_bearish:
                            adjust_type_str = "음봉 조정" if is_candle_bearish else "숨고르기 공방"
                            squeeze_info_str = (
                                f"<br>• ⚠️ <b>[성벽 위 {adjust_type_str}/차익매물출회({bandwidth:.1f}%)]</b> 5일선 위 안착 상태이나 당일 고점 차익 매물이 출회 중이오."
                            )
                        else:
                            squeeze_info_str = (
                                f"<br>• 🟢 <b>[밴드폭 응축돌파({bandwidth:.1f}%)]</b> 5일선을 뚫고 올라섰네! 상방 분출 초입으로 유효하오."
                            )
                else:
                    is_bandwidth_ok = False
                    ...
                    squeeze_info_str = (
                        f"<br>• ⏳ <b>[밴드폭 응축({bandwidth:.1f}%)]</b> 5일선 돌파 전이오. 안착 신호를 기다리시게."
                    )
            else:
                is_bandwidth_ok = True
                ...
                squeeze_info_str = (
                    f"<br>• 🌊 <b>[밴드폭 넉넉함({bandwidth:.1f}%)]</b> 상하 진폭 활주로는 충분히 트였으나, 위 지표 조건 충족 시에만 진격하시게."
                )
            ...
            if not is_below_ma5:
                ...
                stop_loss_label = (
                    "🛡️ 단기 추세 체크포인트: 5일선"
                    f" -{dynamic_stop_pct:.1f}% 이탈 시 비중 조절 및 관망({stop_loss_price:{fmt_p}}{currency})"
                )
            else:
                stop_loss_price = prev_low
                stop_loss_label = (
                    "🚨 칼손절 경보: 바닥권 전저점 이탈 마지노선({stop_loss_price:{fmt_p}}{currency})"
                )
            ...
            if is_pre_market_mode:
                ...
                if not is_down_trend_v:
                    ...
                    else: v_status, v_adv = ("차익투매주의", f"⚠️ <b>[고점 차익투매 경계]</b> 시간보정 강도 {vol_strength:.1f}점! 정배열 상승 속 차익 매물 출회 중이니 5일선 지지를 확인하시게.")
                ...
                if p < ma5_val: v_status, v_adv = ("거래량 미달", f"🟡 <b>[거래량 미달 / 관망]</b> 실시간 {vol_strength:.1f}점! 5일선 아래이므로 섣부른 진입을 엄금하네.")
                else:
                    is_healthy_volume_dry = p_chg >= 0
                    if is_healthy_volume_dry: v_status, v_adv = ("거래 숨고르기", f"🧊 <b>[거래 숨고르기]</b> 시간보정 강도 {vol_strength:.1f}점! 정배열 성벽 위 눌림목 숨 고르는 중이오니 지지력을 관망하시게.")
            ...
            is_escape_buy_signal = (
                ...
                and is_bandwidth_ok and is_orderbook_safe and (not is_target_reached)
            )
        
            is_pullback_buy_signal = (
                (not is_down_trend_structural) and is_ma20_buffer_safe
                and is_ma5_safe and is_orderbook_safe
            ...
            if is_stop_loss_triggered:
                ...
                col = "#6A1B9A"
                final_adv = f" • <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). <b>[볼린저 상단 상방 확장]</b> 현재가({p:{fmt_p}}{currency})가 밴드 상단을 타고 뻗어 나가는 중이오! <b>물량의 50%는 1차 익절하여 수익을 확정</b>하고, <b>잔여 50%는 5일선 이탈 전까지 목표선 상향을 즐기며 홀딩</b>하시게."
                ...
                if is_candle_bearish or (is_positive_day and vol_strength < 65 and not is_pre_market_mode):
                    ...
                    col = "#D32F2F"
                    final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f})점. <b>[성벽 위 {candlestick_type_str}]</b> 현재가({p:{fmt_p}}{currency})가 성벽 위에서 차익 매물 출회 중이오. 추격 매수를 엄금하고 선제적 분할 익절(수확)을 준비하시게."
                elif p >= (target_price_100 * 0.98):
                    final_code = "RED_SELL_WARNING"
                    sig = "🔴 [성벽 위 저항 익절] 목표선 터치 후 위꼬리 이탈!"
                    col = "#D32F2F"
                    final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f})점. <b>[성벽 위 저항 익절]</b> 현재가({p:{fmt_p}}{currency})가 수확 목표선 터치 후 위꼬리를 달고 밀려 내려왔소! 서둘러 매도(수확)를 집행하시게."
                else:
                ...
                action_time_guide = "오전장 화력(300점 이상) 및 5일선 지지 확인 시 10% 선제 타진하시게." if (is_kr and is_morning_breakout_fast) else "14:00 이후 지지 확인 시 10% 분할 타진하시게." if is_kr else "정규장(세션) 지지 확인 시 10% 분할 타진하시게."
                final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). <b>[진바닥 포착 완료]</b> 지표 충족 및 5일선 안착! 비중 10% 수준의 1단계 분할 입절(매수)로 기민하게 접근하시게. {action_time_guide}"
                ...
                action_guide = "오전장 수급(300점 이상)과 5일선 안착 시 즉시 50% 분할 진입 가동하시게." if (is_kr and is_morning_breakout_fast) else "14:00 이후 5일선 안착 확인 시 50% 분할 진입하시게." if is_kr else "정규장(세션) 수급 동반 및 5일선 안착 시 50% 분할 진입하시게."
                ...
                action_guide = "오전장 화력 속 안전마진 안착 시 즉시 50% 타진 가동하시게." if (is_kr and is_morning_breakout_fast) else "14:00 이후 5·20일선 안전마진 안착 시 50% 분할 타진하시게." if is_kr else "정규장(세션) 5·20일선 안전마진 안착 시 50% 분할 타진하시게."
                final_adv = f" • <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). <b>[{time_tag_ok}]</b> {bw_diag_msg}. {action_guide}"
            elif (
                p >= ma20_safe_threshold and (not is_down_trend_structural)
                and is_ma5_safe and pullback_rebound_score >= 1
                and (vol_strength >= 65 or is_pre_market_mode)
            ):
                ...
                final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). <b>[20일선 돌파 안착 타진]</b> 현재가({p:{fmt_p}}{currency})가 20일선 안전마진선({ma20_safe_threshold:{fmt_p}}{currency})을 돌파하였소! {action_guide}"
            elif is_ma_tangled:
                if is_ma5_safe and vol_strength >= 65.0 and bandwidth >= 10.0 and (rsi_val <= 40 or temp_bottom_score >= 1 or (p_chg >= 0.0 and p >= mid_line)):
                    ...
                    final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). <b>[이평선 꼬임 속 진바닥 포착]</b> 수급과 밴드폭이 뒷받침된 상태에서 5일선 안착 및 냉골 바닥 지표가 충족되었으므로, 비중 10% 분할 입절로 냉정하게 대응하시게."
                else:
                ...
                col = "#F57C00"
                final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). 현재 하락 음봉 국면이오니 헛된 기대를 버리고 성벽 및 5일선 이탈 방어에 집중하시게."
                ...
                col = "#F57C00"
                final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). <b>[매수 조건 미충족]</b> 5일선 위에 안착했으나 수급 불량 또는 눌림목 동조 점수 부족({pullback_rebound_score}/3점)으로 관망하시게."
            else:
            ...
            st.markdown(
                ...
                f"<div class='signal-subtext'>{final_adv}</div></div>",
                unsafe_allow_html=True,
            )
            ...
            if final_code == "BREAK_MA20_CONFIRMED":
                ...
                if pullback_rebound_score >= 2: _p_action = f"-> <b>[눌림목 공방 유효]</b> 중간지대 조건 충족!"
            ...
            if is_band_riding:
                ma5_guide_text = f"현재가({p:{fmt_p}}{currency})가 볼린저 상단을 타고 확장 중이오! 50% 익절 완료 후 남은 50%는 <b>5일선({ma5_val:{fmt_p}}{currency}) 종가 이탈 전까지</b> 끝까지 추종하시게."
            elif is_target_reached or p >= (target_price_100 * 0.98):
                ma5_guide_text = f"현재가({p:{fmt_p}}{currency})가 5일선({ma5_val:{fmt_p}}{currency}) 위에 있으나 수학 목표선에 도달했으므로 5일선 -{dynamic_stop_pct:.1f}% 이탈({ma5_dynamic_stop:{fmt_p}}{currency})을 잔여 물량 정리선으로 엄수하시게."
                ...
                elif is_down_trend_v or is_candle_bearish: ma5_guide_text = f"현재가({p:{fmt_p}}{currency})가 5일선({ma5_val:{fmt_p}}{currency}) 위에 안착해 있으나, 당일 음봉/조정 중이오. 5일선 사수 확인 후 대응하시게."
            ...
            if is_band_riding:
                def_status = f"성벽({defense_line:{fmt_p}}{currency})을 가뿐히 넘어 볼린저 상단이 찢어지고 있네! 든든한 방어선을 뒤에 두고 잔여 추세를 즐기시게."
            elif is_target_reached or p >= (target_price_100 * 0.98):
                def_status = f"성벽({defense_line:{fmt_p}}{currency}) 위 진격은 완수되었네! 수학 목표선({target_price_100:{fmt_p}}{currency}) 코앞이니 현금을 챙기시게."
                ...
                if is_candle_bearish or (is_positive_day and vol_strength < 65 and not is_pre_market_mode):
                    def_status = f"성벽({defense_line:{fmt_p}}{currency}) 위 안착 중이나 당일 차익 매물 출회 및 숨고르기 공방 중이오! 무리한 추격을 삼가시게."
                else:
                    def_status = f"성벽({defense_line:{fmt_p}}{currency}) 위에서 <b>상방 랠리 진격 중</b>이네! 방어선을 등지고 추세를 즐기시게."
            else:
                if final_code == "BREAK_MA20_CONFIRMED": def_status = f"성벽({defense_line:{fmt_p}}{currency}) 아래이나 <b>20일선 돌파 안착</b>에 성공하여 기민한 타점을 형성 중이네!"
            ...
            # ★ [v36106 누락 보수] MACD 기초 텍스트 1차 정의 (상/하단 공용 변수 무결점화)
            ...
            # ★ 1. 상단 요약 전광판용 MACD 텍스트 조립
            if is_band_riding:
                macd_strategy_msg = "<b>🔥 엔진 풀가동 + 밴드 라이딩</b><br>• <b>역할:</b> 상방 대시세 추종.<br>• <b>진단:</b> 엔진 가속과 함께 상단 밴드가 열리고 있소! 추격 매수는 자제하되, 1차 절반 익절 후 남은 물량은 5일선 이탈 전까지 강하게 끌고 가시게."
            elif is_target_reached or p >= (target_price_100 * 0.99):
                macd_strategy_msg = "<b>🚨 엔진 과열 경보 (수학 목표선 도달)</b><br>• <b>역할:</b> 상단 오버슈팅 방어.<br>• <b>진단:</b> 엔진 가속도가 붙어 있어도 상단 저항선 코앞일세! 추격 매수는 엄금이며, 1~2호가 아래에 매도 주문을 깔아두어 이익을 챙기시게."
            elif is_overall_cautious_state and not (is_kr and is_morning_breakout_fast):
                macd_strategy_msg = f"{base_macd_desc}<br>• <b>[관망 기조 동조]:</b> 현재 상단 종합 결론이 관망/경계 상태이므로, 엔진 상태와 무관하게 섣부른 추격매수를 금하고 안전하게 관망하시게."
            else:
            ...
            with i1:
                if is_long_upper_tail: bb_diag = "🟡 <b>[위꼬리 저항 관망 구역]</b><br>• <b>진단:</b> 긴 위꼬리가 밀려 내려왔으니 섣부른 추격을 금하고 관망."
                elif final_code == "BOTTOM_ENTRY": bb_diag = f"🔴 <b>[1단계 진바닥 입질 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 지표 터치 + 바닥 지지 확인! 10% 선제 타진."
                elif final_code == "ESCAPE_BUY": bb_diag = f"🟢 <b>[2단계 진바닥 탈출 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> {bw_diag_msg}. 50% 분할 진입 유효."
                elif final_code == "BREAK_MA20_CONFIRMED": bb_diag = f"🔵 <b>[20일선 돌파 안착 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 20일선 돌파 및 지표 동조 성공! 분할 진입 가동."
                elif final_code == "WAIT_VOLUME": bb_diag = f"🟡 <b>[수급 대기 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 거래량이 부족하니 진입 보류."
                elif final_code == "WAIT_DOWNTREND_FALL": bb_diag = f"🟡 <b>[진바닥 탐색/칼날 관망 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 하락 구간이오. 회복 전까지 관망."
                elif final_code == "WAIT_PULLBACK_CANDLE": bb_diag = f"🟡 <b>[5일선 지지 검증 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 당일 숨고르기/음봉 중이오. 5일선 지지 사수 확인."
                elif final_code == "PULLBACK_BUY": bb_diag = f"🔵 <b>[3단계 눌림목 추가 매수 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> {bw_diag_msg}. 50% 분할 진입 유효."
                elif final_code == "BAND_RIDING_HARVEST": bb_diag = f"🟣 <b>[밴드 라이딩 대시세 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 상단 밴드 확장 중! 50% 이익 확정 후 잔량 홀딩."
                elif final_code == "RED_SELL_TARGET": bb_diag = f"🔴 <b>[수학 목표선 저항 도달 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 상단 저항 도달! 물량의 50% 분할 매도."
                elif final_code == "RED_SELL_WARNING": bb_diag = "🔴 <b>[성벽 위 익절 구간]</b><br>• <b>진단:</b> 성벽 위 차익 매물 출회로 분할 익절 실행."
                elif final_code == "BREAKOUT_ATTACK": bb_diag = f"🟢 <b>[성벽 위 진격 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 성벽을 뚫고 목표선({target_price_100:{fmt_p}}{currency})을 향해 진격 중."
                elif final_code == "YELLOW_CAUTION": bb_diag = "🟡 <b>[성벽 위 경계 및 추격 차단 구역]</b><br>• <b>진단:</b> 성벽 위 공방 중이므로 추격 매수 금지."
                elif final_code == "WAIT_OVER_EXTENDED": bb_diag = f"🟡 <b>[과다이격 추격 금지 구역] (5일선 이격: +{bias_ma5:.1f}%)</b><br>• <b>진단:</b> 5일선 대비 5% 이상 벌어졌으니 매수 보류."
                elif final_code in ["WAIT_MA20_BUFFER", "WAIT_ORDERBOOK"]: bb_diag = f"🟡 <b>[호가/20일선 검증 대기 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 매도잔량 우세 및 안착 미확인으로 진입 보류."
                else: bb_diag = f"⚖️ <b>[관망 및 대기 구역] (밴드폭: {bandwidth:.1f}%)</b><br>• <b>진단:</b> 지표 동조 미흡으로 안착 대기 중."
            ...
            with i2:
                ...
                if is_target_reached or rsi_val >= 60: r_status = "<b>👿 불지옥 과열권</b><br>• <b>진단:</b> 과열 구간 진입, 상단 차익 실현을 준비하시게."
                elif rsi_val <= 38: r_status = "<b>🧊 냉골 바닥권</b><br>• <b>진단:</b> 바닥권 지표 터치 및 수급 유입 대기."
                else: r_status = "<b>⚖️ 적정 온도 구간</b><br>• <b>진단:</b> 에너지 충전 중. 보조지표 고개 돌림 주시."
            ...
            with i3:
                ...
                if is_target_reached or will_val >= -20: w_status = "<b>🚀 상방 저항 도달 구역</b><br>• <b>진단:</b> 목표선 도달! 추격 매수 엄금 및 선제 익절 대기."
                elif will_val <= -75: w_status = "<b>🏳️ 개미 항복 구역</b><br>• <b>진단:</b> 🧊 <b>[바닥 침체]</b> -75 밑 투매 진행 중! 세력 선취매 주시."
                else: w_status = "<b>⚖️ 중간 지대</b><br>• <b>진단:</b> 상/하방 방향 탐색 중."
            ...
            with i4:
                # ★ 2. 하단 인디케이터용 MACD 텍스트 별도 조립 (오류 보수 완료!)
                if is_band_riding:
                    m_diag = "<b>🔥 엔진 풀가동 (대세 추종)</b><br>• <b>진단:</b> 밴드 확장과 함께 엔진이 힘을 내고 있소! 50% 수확 후 5일선 사수."
                elif is_target_reached or p >= (target_price_100 * 0.99):
                    m_diag = "<b>🚨 엔진 과열 차단</b><br>• <b>진단:</b> 목표선 도달 완료로 추가 가속 중단! 잔여 물량 익절에 집중하시게."
                elif is_overall_cautious_state and not (is_kr and is_morning_breakout_fast):
                    m_diag = f"{base_macd_desc}<br>• <b>[관망 기조 동조]:</b> 상단 결론이 관망 상태이므로, 섣부른 추격매수를 금하고 관망."
                else:
