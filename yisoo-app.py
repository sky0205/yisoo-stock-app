...
if symbol:
    try:
        ...
        if df.empty:
            ...
            # ==================================================================
            # ★ [신호등 분기 논리 - 냉정하고 무결한 우선순위 규칙]
            # 1. 손절 2. 목표도달/밴드라이딩 3. 혼조세 속 5일선 방어/진바닥 입절 (단, 5일선 방어 및 상승세 충족 시에만 허용) 
            # 4. 이평선 꼬임 혼조세 속 5일선 미방어 시 철저한 관망(노란불) 5. 20일선 저항 하회 시 보류
            # ==================================================================
            try:
            ...
            if is_stop_loss_triggered:
                ...
                final_adv = (
                    f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                    f"<b>[수학 목표선 도달 완료]</b> 볼린저 상단({target_price_100:{fmt_p}}{currency}) 머리를 들이받았소! 우선 50% 물량을 기계적으로 수확하시게."
                )
            elif is_band_riding:
                ...
                final_adv = (
                    f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                    f"<b>[볼린저 상단 상방 확장]</b> 현재가({p:{fmt_p}}{currency})가 볼린저 상단을 타고 위로 솟구치는 중이오! 물량의 50%는 1차 익절하시게."
                )
            elif (bias_ma5 >= 0.0) and (current_chg >= 0.0) and not is_down_trend_v and not is_stop_loss_triggered and not (p < mid_line and is_ma_tangled):
                # 🟢 5일선 위 안착 및 양봉 상승세이고 20일선 저항 아래에 갇힌 극단적 역배열이 아닐 때만 혼조세 속 5일선 방어 입절 포착 허용
                final_code = "BOTTOM_ENTRY"
                sig = "🟢 [혼조세 속 입절 포착] 1단계 분할 매수 유효 구역"
                col = "#388E3C"
                action_time_guide = (
                    "14:00 이후 5일선 및 볼린저 바닥 지지 확인 시 30~50% 분할 타진하고, 15:20 저가 사수 시 완성하시게. (단, 윗꼬리 달고 5일선 이탈 시 즉시 철수)"
                    if is_kr else
                    "07:00 마감 일봉상 5일선 및 바닥선 사수를 확인 후 30~50% 분할 진입하시게. (단, 윗꼬리 이탈 시 즉시 철수)"
                )
                final_adv = (
                    f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                    f"• <b>[이평선 꼬임 속 5일선 방어]</b> 이평선들은 서로 엉켜 있으나 단기 5일선 방어력이 살아 있으므로, 비중 30~50%의 1단계 분할 입절로 냉정하게 대응하시게. {action_time_guide}"
                )
                ...
                final_adv = (
                    f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                    f"• <b>[진바닥 안착]</b> 지표 충족 및 5일선 안착! 비중 30~50% 수준의 1단계 분할 입절로 접근하시게. {action_time_guide}"
                )
                ...
                final_adv = (
                    f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                    f"<b>[{time_tag_ok}]</b> {bw_diag_msg}. 5일선 안착 확인 후 2단계 분할 진입하시게."
                )
                ...
                final_adv = (
                    f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                    f"<b>[{time_tag_ok}]</b> 5·20일선 안전마진 안착! 3단계 완성하시게."
                )
                ...
                if p < mid_line:
                    final_adv = (
                        f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                        f"<b>[20일선 저항 하회]</b> 5일선은 회복했으나 머리 위 20일선({mid_line:{fmt_p}}{currency}) 저항 아래에 갇혀 있소! 20일선 돌파 및 안착 확인 전까지 진입을 보류하시게."
                    )
                else:
                    final_adv = (
                        f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                ...
                # 🟡 5일선 방어를 하지 못했거나 방향성을 완전히 상실한 이평선 꼬임 혼조세 구간
                ...
                final_adv = (
                    ...
                    "상단 저항선 돌파와 확실한 정배열 분출이 확인될 때까지 무리한 추격을 금하고 냉정하게 관망하시게."
                )
            else:
            ...
            if is_band_riding:
                ...
                pullback_action_str = "-> <b>[진바닥 탐색 중]</b> 역배열 하락 진행형"
            else:
                if p < mid_line:
                    pullback_action_str = f"-> <b>[매수 보류]</b> 5일선은 회복했으나 <b>머리 위 20일선({mid_line:{fmt_p}}{currency}) 저항 아래</b>에 갇혀 있음"
                else:
            ...
            indicator_verify_text = (
                ...
                f"&nbsp;&nbsp;&nbsp;- <b>눌림목 지지 동조:</b> {pullback_rebound_score}/3점 {pullback_status_str} {pullback_action_str}{squeeze_info_str}"
            )
            ...
            if is_band_riding:
                holder_guide_msg = "• <b>[밴드 라이딩 대시세 구역]</b><br>• <b>실전 행동:</b> 50% 익절 후 5일선 사수 기준으로 잔여 물량 추종."
            elif is_target_reached:
                holder_guide_msg = "• <b>[수학 목표선 도달 완료 구역]</b><br>• <b>실전 행동:</b> 볼린저 상단 저항선에 닿았으니 50% 분할 매도."
                ...
                holder_guide_msg = f"• <b>[보유자 가이드 (평단가: {user_avg_price:{fmt_p}}{currency} / 수익률: {profit_rate:+.2f}%)]</b><br>• 5일선({ma5_val:{fmt_p}}{currency}) 이탈 여부를 주시하며 대응하시게."

            ma5_guide_text = f"현재가({p:{fmt_p}}{currency})가 5일선({ma5_val:{fmt_p}}{currency}) 위에 안착하여 단기 전투선 유지 중이오. 5일선 사수 여부를 지켜보시게."
            def_status = f"성벽({defense_line:{fmt_p}}{currency}) 위에서 기세를 타고 <b>위로 진격 중</b>이네!"
            macd_strategy_msg = "<b>⚙️ 엔진 점검 중</b><br>• <b>[엔진 연동]:</b> 위 전황에 맞춰 유효하게 대응하시게."
        ...
        if df.empty:
           ...
           # 전일 종가 변수는 위에서 확정한 prev_p를 사용한다.
            ...
            if is_stop_loss_triggered:
                ...
                # 🟡 이평선이 완전히 엉켜 있고 상단 매수 조건에 부합하지 않을 때의 엄격한 관망 진단
                pullback_rebound_score = 0
                bottom_score = min(bottom_score, 0)
                final_code = "MA_TANGLED_WARNING"
                sig = "🟡 [이평선 꼬임 혼조세] 방향성 상실로 인한 관망"
