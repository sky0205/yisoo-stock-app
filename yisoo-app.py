...
if symbol:
    try:
        ...
        if is_kr:
            ...
            # 만약 네이버 가격을 못 가져왔으면 야후 차트 종가로 대체
        ...
        # 만약 끝내 데이터가 없으면 안내 문구 출력 후 중단
        if p == 0 or df is None or df.empty or 'Close' not in df.columns:
            st.error(f"⚠️ 종목번호({symbol})의 실시간 데이터를 불러오지 못했습니다. 올바른 번호인지 확인하시게.")
        else:
