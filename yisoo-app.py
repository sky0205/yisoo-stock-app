...
# 1. 화면 구성 및 디자인 (신호등 및 수치 강조)
st.set_page_config(page_title="v36056 냉정진단기 Final", layout="wide")
st.markdown("""
    <style>
    .stApp { background-color: #ECEFF1; } 
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    .global-box { background-color: #263238; color: #FFFFFF; padding: 15px; border-radius: 12px; border: 2px solid #455A64; margin-bottom: 15px; }
    .global-title { font-size: 20px !important; color: #81D4FA !important; margin-bottom: 5px; }
    .global-diag { font-size: 15px !important; color: #CFD8DC !important; line-height: 1.4; }
...
def get_global_full(ticker, name, comment_up, comment_down):
    try:
        ...
        return f"""<div class='global-box'><div class='global-title'>{name}</div>
                   <span style='font-size:26px; color:{color};'>{curr:,.2f} ({pct:+.2f}%)</span><br>
                   <div class='global-diag'><b>👴 평가:</b> {diag}</div></div>"""
    except: return f"<div class='global-box'>{name}<br>장부 확인 불가</div>"

g1.markdown(get_global_full("^IXIC", "나스닥 (NASDAQ)", "기술주 거품이 여전하네. 가짜 반등이야.", "금리 압박에 숨이 턱턱 막히는 형국일세."), unsafe_allow_html=True)
g2.markdown(get_global_full("^GSPC", "S&P 500", "큰 집들이 체면치레 중이나 안개는 안 걷혔어.", "시장 전체가 도살장 분위기로 변하고 있구먼."), unsafe_allow_html=True)
g3.markdown(get_global_full("^TNX", "미국채 10년 금리", "이놈이 튀면 주식은 곡소리 나는 법이야.", "잠시 숨을 고르나 언제든 비수가 될 놈일세."), unsafe_allow_html=True)
...
st.write("---")
st.caption("분석가 서강윤: 2026년 실시간 장부 및 v36056 최종 양식(신호등 복구완료) 적용")
