...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [수정] 네 기둥 지수 상세 진단 부활 (Bollinger, RSI, MACD, Williams %R)
            ...
            # 상단 헤더 및 거래량 박스
            # [수정] 전일비 천 단위 콤마 표시 (fmt_change 추가)
            if currency == "원":
                fmt_p = ",.0f"; fmt_chg = "+,.0f"
            else:
                fmt_p = ",.2f"; fmt_chg = "+,.2f"
            ...
            st.markdown(f"""<div class='trend-card'>
                ...
                <div class='trend-item'>● <b>필살 조언:</b> <span class='advice-highlight'>{'⚠️ 신고가 추격 시: ' + format(p*0.95, fmt_p) + ' ' + currency + ' 이탈 시 손절!' if is_new_high else '📉 신저가 구간: ' + format(p, fmt_p) + ' ~ ' + format(year_low*0.95, fmt_p) + ' 사이 3회 분할 매수!' if is_new_low else '낚싯대만 던져두고 지표 바닥권을 기다리십시오.'}</span></div>
            ...
            # 네 기둥 지수 상세 진단 부활 (Bollinger/RSI/Williams/MACD)
            # Bollinger 상세 진단 보완
