...
def display_global_risk():
    ...
    try:
        ...
        advice = f"✅ **[미장 쾌청: 진격!]** 기세 좋구먼." if n_chg > 0.5 else f"🚨 **[긴급 상황: 정박!]** 피신하시게." if n_chg < -1.0 else "🧐 **[안개 정국: 관망]** 기다리시게."
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [복구] 거래량 상세 판독 (시초 30% 로직 포함)
            v_status = "💤 거래침체" if v_ratio < 100 else "📈 거래증가" if v_ratio < 200 else "🔥 거래폭발"
            if current_hour <= 11 and v_ratio > 30 and p_chg > 2:
                v_adv = "🔥 **[시초 세력 진격!]** 시초장에 벌써 거래량이 30%를 넘으며 주가 급등 중일세! 세력이 돈 쓰고 밀어 올리는 기세니 빳빳하게 기세 타시게!"
            elif p_chg > 3 and v_ratio > 150: v_adv = "🔥 **[주가급등+거래폭발]** 세력이 돈을 써서 성벽을 뚫고 있구먼! 진격할 자리일세!"
            ...
            with i1: # Bollinger
                bb_diag = f"● **[비상: 성벽 돌파!]** 하단 성벽 아래일세! 지금은 지지 확인 후 진격 기회를 볼 때구먼!" if p <= low_b else f"● 지지 확인 전까지 낚싯대만 던지시게."
