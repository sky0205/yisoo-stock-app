...
# 1. 화면 구성 및 할배 캐릭터 스타일 (제목 박스 통합 및 가독성 극대화)
...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    /* [제목 통합형 박스 스타일] */
    .section-box { background-color: #FFFFFF; padding: 25px; border-radius: 15px; border: 3px solid #CFD8DC; box-shadow: 0 4px 6px rgba(0,0,0,0.1); margin-bottom: 25px; }
    .section-title { font-size: 32px !important; color: #1565C0 !important; border-bottom: 4px solid #E3F2FD; padding-bottom: 12px; margin-bottom: 20px; display: block; }
    
    .stock-header { background-color: #FFFFFF; padding: 20px; border-radius: 12px; border-left: 12px solid #1E88E5; margin-bottom: 15px; }
    .price-main { font-size: 52px !important; color: #D32F2F !important; line-height: 1.2; }
    
    .vol-main-text { font-size: 38px !important; color: #0D47A1 !important; margin-bottom: 12px; }
    .vol-sub-text { font-size: 26px !important; color: #1565C0 !important; line-height: 1.6; background-color: #F5F5F5; padding: 18px; border-radius: 10px; border-left: 12px solid #1E88E5; }
    
    .signal-box { padding: 35px; border-radius: 20px; text-align: center; margin-bottom: 20px; }
    .signal-text { font-size: 80px !important; font-weight: 900 !important; color: #FFFFFF !important; }
    
    .trend-card { background-color: #FFFFFF; padding: 35px; border-radius: 25px; border: 8px solid #D32F2F; margin: 25px 0; }
    .trend-title { font-size: 40px !important; color: #D32F2F !important; border-bottom: 5px solid #FFEBEE; padding-bottom: 15px; margin-bottom: 25px; }
    .trend-item { font-size: 28px !important; line-height: 1.8; margin-bottom: 18px; }
    
    /* 지수 훈수 박스 글씨 대폭 강화 */
    .ind-box { background-color: #FFFFFF; padding: 28px; border-radius: 20px; border: 4px solid #90A4AE; min-height: 520px; margin-bottom: 20px; }
    ...
    .ind-diag { font-size: 28px !important; color: #333333 !important; line-height: 1.8; background-color: #FAFAFA; padding: 22px; border-radius: 12px; border-left: 14px solid #D32F2F; }
    
    [data-testid="stMetricValue"] { font-size: 50px !important; font-weight: 900 !important; color: #212121 !important; }
    [data-testid="stMetricLabel"] { font-size: 26px !important; color: #546E7A !important; }
    </style>
    """, unsafe_allow_html=True)

# 글로벌 지표 (제목을 박스 안으로 통합)
def display_global_risk():
    try:
        ...
        st.markdown("<div class='section-box'><span class='section-title'>🌍 글로벌 시장 종합 전황</span>", unsafe_allow_html=True)
        ...
        c3.metric("미 국채 10년 (TNX)", f"{tnx_val:.2f}%", f"{tnx_chg:+.2f}%")
        if n_chg > 0.5 and tnx_chg < 0: advice = "✅ **[미장 쾌청: 진격!]** 기세 타고 진격하시게."
        elif n_chg < -1.0: advice = "🚨 **[긴급 상황: 정박!]** 성벽 무너지기 전에 피신하시게."
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            st.markdown(f"<div class='stock-header'><p style='font-size:32px; color:#1565C0; margin:0;'>{name} ({symbol})</p><p class='price-main'>{format(p, fmt_p)} {currency} <span style='font-size:35px;'>({p_chg:+.2f}%)</span></p></div>", unsafe_allow_html=True)
            
            # [복구 완벽] 거래량 상세 판독 (제목을 박스 안으로 통합)
            st.markdown("<div class='section-box'><span class='section-title'>📊 실시간 거래량 전황 분석</span>", unsafe_allow_html=True)
            v_label = "💤 거래침체" if v_ratio < 100 else "📈 거래증가" if v_ratio < 200 else "🔥 거래폭발"
            if v_ratio >= 30 and is_opening:
                v_status = f"🔥 시초 거래폭발 ({v_ratio:.1f}%)"
                v_adv = f"🔥 **[세력 진격!]** 거래량이 5일 평균 대비 {v_ratio:.1f}% 터지며 폭등 중일세! 진짜 세력이 미는 거니 빳빳하게 기세 타시게!" if p_chg >= 3 else f"💀 **[비명 포착!]** 거래량이 {v_ratio:.1f}% 터지며 폭락 중일세! 성벽 함락 중이니 일단 피신하시게!" if p_chg <= -3 else f"✅ 거래량 터졌으나 주가가 힘겨루기 중일세. 눈을 부라리고 보시게."
            else:
            ...
            # [복구 완벽] 성벽 가격선 (제목을 박스 안으로 통합)
            st.markdown("<div class='section-box'><span class='section-title'>🛡️ 매수·매도 핵심 성벽 가격선</span>", unsafe_allow_html=True)
            c1, c2, c3 = st.columns(3)
            ...
            adv2 = f"2. **성벽 사수 확인:** 성벽({format(defense_line, fmt_p)}) {'함락됐으니 지하실 조심하시게.' if p < defense_line else '사수 중이니 진격의 발판 삼으시게.'}"
            adv3 = f"3. **엔진(MACD) 확인:** 엔진이 {'정회전 중일세!' if m_l > s_l else '**역회전 중!** 절대 속지 마시게.'}"
            ...
            elif m_l < s_l or p < defense_line: final_adv = "🧐 **[최종 결론] 미동도 말고 관망하며 기다리시게!**"
            ...
            st.markdown(f"""<div class='trend-card'><div class='trend-title'>⚔️ {name} 실전 필살 대응 전략</div>
                ...
                <hr style='border:2px solid #FFEBEE;'><div class='trend-item' style='color:#D32F2F; font-size:38px !important;'>{final_adv}</div></div>""", unsafe_allow_html=True)

            # [복구 완벽] 네 기둥 지수 상세 분석 (비수 꽂는 매서운 훈수)
            st.divider()
