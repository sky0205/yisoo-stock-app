        ...
        if not df.empty:
            ...
            if v_ratio >= 30 and is_opening:
                if p_chg >= 3:
                    ...
                    v_adv = f"🔥 **[세력 진격!]** 시초 거래량이 {v_ratio:.1f}% 터지며 주가 폭등 중일세! 빳빳하게 기세 타시게!"
                elif p_chg <= -3:
                    v_status = f"💀 현지 시초장 주가 폭락 / 거래 폭발 ({v_ratio:.1f}%)"
                    v_adv = f"💀 **[비명 포착!]** 시초 거래량이 {v_ratio:.1f}% 터지며 폭락 중일세! 피신하시게!"
                else:
            ...
            # [핵심 수선] 네 기둥 지수 상세 훈수 (RSI 판독 쐐기박기)
            st.divider()
            i1, i2, i3, i4 = st.columns(4)
            with i1: # Bollinger
                if p >= up_b: bb_diag = f"● **[비상: 상단 성벽 돌파!]** 가격이 하늘을 찌르는구먼! 이제는 수확(익절) 준비를 하시게."
                elif p <= low_b: bb_diag = f"● **[비상: 하단 성벽 돌파!]** 바닥권일세! 여기서 지지받으면 진짜 진격 기회일세."
                else: bb_diag = f"● 중앙선 {'위' if p > mid_line else '아래'}일세. 성벽 사수 여부를 매섭게 보시게."
            ...
            with i2: # RSI (어르신 지적 완벽 반영)
                if rsi_val >= 60: r_diag = f"● 지수 {rsi_val:.2f}로 **👺 불지옥(과열)** 문턱일세! 천장에 다 왔으니 탐욕 버리고 익절가 빳빳하게 잡으시게."
                elif rsi_val <= 35: r_diag = f"● 지수 {rsi_val:.2f}로 **🧊 냉골(과매도)** 상태일세! 남들 무서울 때 우리는 냉정하게 바닥을 보시게."
                else: r_diag = f"● 현재 지수 {rsi_val:.2f}로 눈치싸움 중일세. 섣불리 진격하지 말고 지표 끝단을 기다리시게."
            ...
            with i3: # Williams
                w_diag = f"● 지수 {will_val:.2f}로 **🏳️ 개미 항복 구간**일세! 고개 들면 무조건 진격일세!" if will_val < -80 else f"● 지수 {will_val:.2f}로 **🧨 천장 광기 구간**일세! 언제 비수 꽂힐지 모르니 매섭게 보시게." if will_val > -20 else f"● 현재 중간지대일세."
            ...
            with i4: # MACD
                m_diag = f"● 엔진 정회전 중! 기세 붙었으니 성벽 사수 보시게." if m_l > s_l else f"● 엔진 **역회전** 중이네! 거꾸로 도는 차엔 타지 마시게."
