...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [복구 2] 매수·매도 핵심 성벽 가격선 복구 (신호등 바로 아래)
            ...
            with i1: st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>Bollinger (기세)</p><p class='ind-diag'>● **[상단 돌파!]** 하늘 찌르네! 익절 준비 하시게. 소나기가 올 수 있으니 보따리 싸시게.</p></div>", unsafe_allow_html=True)
            ...
            with i3:
                ...
                st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>Williams %R</p><p style='font-size:40px; color:#E65100;'>{will_val:.1f}</p><p class='ind-diag'>● 지수 {will_val:.1f}로 **항복 지점** 근처입니다. 보따리를 푸십시오.</p></div>", unsafe_allow_html=True)
