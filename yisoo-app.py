...
# 1. 화면 구성 및 할배 캐릭터 스타일 (박스 전면 제거 및 가독성 극대화)
...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    /* [어르신 특명] 모든 박스 테두리를 없애고 시원한 글씨만 남기는 스타일 */
    .clean-title {
    ...
    /* 주가 헤더 스타일 */
    .stock-header { padding: 25px 0; margin-bottom: 25px; border-bottom: 3px solid #CFD8DC; }
    .price-main { font-size: 60px !important; color: #D32F2F !important; line-height: 1.1; }
    
    /* 거래량 상세 훈수 스타일 (배경 박스 제거) */
    .vol-main-text { font-size: 45px !important; color: #0D47A1 !important; margin-bottom: 15px; }
    .vol-sub-text { font-size: 32px !important; color: #1565C0 !important; line-height: 1.8; padding: 20px 0; }
    
    /* 신호등 및 전략 카드 */
    .signal-box { padding: 50px; border-radius: 20px; text-align: center; margin-bottom: 35px; }
    .trend-card { padding: 40px 0; margin: 30px 0; border-top: 5px solid #D32F2F; border-bottom: 5px solid #D32F2F; }
    .trend-title { font-size: 48px !important; color: #D32F2F !important; margin-bottom: 30px; }
    .trend-item { font-size: 35px !important; line-height: 1.8; margin-bottom: 25px; }
    
    /* 지수 훈수 상세 (박스 대신 구분선 사용) */
    .ind-section { padding: 35px 0; border-bottom: 2px solid #CFD8DC; min-height: 450px; }
    .ind-title { font-size: 40px !important; color: #1976D2 !important; margin-bottom: 20px; }
    .ind-diag { font-size: 32px !important; color: #333333 !important; line-height: 1.8; border-left: 15px solid #D32F2F; padding-left: 20px; }
    
    /* 대왕 글씨 metric 수치 대폭 강화 */
    [data-testid="stMetricValue"] { font-size: 75px !important; font-weight: 900 !important; }
    [data-testid="stMetricLabel"] { font-size: 35px !important; color: #546E7A !important; }
    </style>
    """, unsafe_allow_html=True)

# 글로벌 전황: 박스 싹 없애고 글씨 제목과 수치만 시원하게
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            st.markdown(f"<div class='stock-header'><p style='font-size:40px; color:#1565C0; margin:0;'>{name} ({symbol})</p><p class='price-main'>{format(p, fmt_p)} {currency} <span style='font-size:42px;'>({p_chg:+.2f}%)</span></p></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<div class='signal-box' style='background-color:{col};'><p style='font-size:100px; font-weight:900; color:white; margin:0;'>{sig}</p><p style='color:white; font-size:48px;'>{s_adv}</p></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"""<div class='trend-card'><div class='trend-title'>⚔️ {name} 실전 필살 대응 전략</div>
                ...
                <hr style='border:2px solid #FFEBEE;'><div class='trend-item' style='color:#D32F2F; font-size:45px !important;'>{final}</div></div>""", unsafe_allow_html=True)

            # [복구 완벽] 4대 지수 상세 분석 (박스 없이 매서운 훈수)
            st.divider()
            ...
            with i1: # Bollinger
                ...
                st.markdown(f"<div class='ind-section'><p class='ind-title'>Bollinger (기세)</p><p class='ind-diag'>{bb_diag}</p></div>", unsafe_allow_html=True)
            with i2: # RSI
                ...
                st.markdown(f"<div class='ind-section'><p class='ind-title'>RSI (온도)</p><p style='font-size:80px; color:#E65100;'>{rsi_val:.1f}</p><p class='ind-diag'>{r_diag}</p></div>", unsafe_allow_html=True)
            with i3: # Williams
                ...
                st.markdown(f"<div class='ind-section'><p class='ind-title'>Williams %R</p><p style='font-size:80px; color:#E65100;'>{will_val:.1f}</p><p class='ind-diag'>{w_diag}</p></div>", unsafe_allow_html=True)
            with i4: # MACD
                m_diag = "● 엔진이 정회전 중일세! 기세 붙었으니 성벽 사수 보시게." if m_l > s_l else f"● 엔진이 **역회전** 중이네! 거꾸로 도는 차에 올라타면 안 되네. 절대 속지 마시게."
                st.markdown(f"<div class='ind-section'><p class='ind-title'>MACD (엔진)</p><p class='ind-diag'>{m_diag}</p></div>", unsafe_allow_html=True)
