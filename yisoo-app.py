    ...
    try:
        ...
        if not df.empty:
            ...
            if v_ratio >= 30 and is_opening:
                if p_chg >= 3: v_status, v_adv = f"🔥 현지 시초 주가폭등 / 거래폭발 ({v_ratio:.1f}%)", f"🔥 **[세력 진격!]** 거래량이 5일 평균 대비 {v_ratio:.1f}% 터지며 폭등 중일세! 빳빳하게 기세 타시게!"
                elif p_chg <= -3: v_status, v_adv = f"💀 현지 시초 주가폭락 / 거래폭발 ({v_ratio:.1f}%)", f"💀 **[비명 포착!]** 시초 거래량이 {v_ratio:.1f}% 터지며 폭락 중일세! 성벽 함락 중이니 피신하시게!"
                else: v_status, v_adv = f"📈 현지 시초 거래급등 ({v_ratio:.1f}%)", "✅ 거래량은 터졌는데 주가가 힘겨루기 중일세. 눈을 부라리고 보시게."
            else:
                v_label = "💤 거래침체" if v_ratio < 100 else "📈 거래증가"
                v_status = f"{v_label} ({v_ratio:.1f}%)"
                if p_chg > 3 and v_ratio < 100: v_adv = f"🚨 **[가짜 상승 주의!]** 주가는 {p_chg:.2f}%나 올랐는데 거래량은 {v_ratio:.1f}%로 빈 수레일세! 속지 마시게."
            ...
            # [완벽 복구] 네 기둥 지수 상세 분석 (비수 꽂는 매서운 설명)
            st.divider()
            i1, i2, i3, i4 = st.columns(4)
            with i1: # Bollinger
                if p >= up_b: bb_diag = f"● **[비상: 상단 성벽 돌파!]** 하늘 찌르는구먼! 이제는 탐욕 버리고 수확(익절) 준비 하시게."
                elif p <= low_b: bb_diag = f"● **[비상: 하단 성벽 돌파!]** 바닥권일세! 지지받고 고개 들면 진짜 진격 기회일세."
                elif p < mid_line: bb_diag = f"● 중앙선 아래서 빌빌대고 있구먼. 성벽 사수 확인 전까지는 낚싯대만 던지시게."
                else: bb_diag = f"● 중앙선 위에서 성벽 사수 중일세. 상단 돌파 여부를 매섭게 보시게."
            ...
            with i2: # RSI (60 이상 불지옥 반영)
                r_diag = f"● 지수 {rsi_val:.2f}로 **👺 불지옥** 문턱일세! 천장에 다 왔으니 익절가 빳빳하게 잡으시게." if rsi_val >= 60 else f"● 지수 {rsi_val:.2f}로 **🧊 냉골** 상태일세! 남들 무서울 때 우리는 냉정하게 바닥을 보시게." if rsi_val <= 35 else f"● 탐욕과 공포 사이 중립 기어 넣고 눈치싸움 중일세. 지표 끝단을 기다리시게."
            ...
            with i3: # Williams
                w_diag = f"● 지수 {will_val:.2f}로 **🏳️ 개미 항복** 구간일세! 바닥 끝단이니 고개 들면 무조건 진격일세!" if will_val < -80 else f"● 지수 {will_val:.2f}로 **🧨 천장 광기** 구간일세! 언제 비수 꽂힐지 모르니 매섭게 보시게." if will_val > -20 else f"● 현재 중간지대일세. 바닥인지 천장인지 갈피를 못 잡고 있구먼."
