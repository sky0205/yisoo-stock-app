...
# 1. 화면 구성 및 디자인 (가장 시원시원한 스타일)
st.set_page_config(page_title="v36056 냉정진단기 Final", layout="wide")
st.markdown("""
    <style>
    .stApp { background-color: #ECEFF1; } 
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    .global-card { background-color: #263238; color: #FFFFFF; padding: 15px; border-radius: 10px; text-align: center; border: 1px solid #455A64; margin-bottom: 10px; }
    ...
    .val-main { font-size: 38px !important; color: #D32F2F !important; margin: 10px 0; display: block; }
    .ind-box { background-color: #FFFFFF; padding: 25px; border-radius: 15px; border: 2.5px solid #90A4AE; min-height: 400px; margin-bottom: 15px; }
    ...
    .ind-value { font-size: 55px !important; color: #B71C1C !important; text-align: center; display: block; margin: 10px 0; }
    </style>
    """, unsafe_allow_html=True)

st.title("👴 이수할아버지의 냉정 진단기 v36056 (미장분석+성벽 복구)")

# 2. 글로벌 전황 (미장 분석 - 수치 위주)
st.subheader("🌎 [미장 분석] 글로벌 주요 지표")
...
def get_global(ticker, name):
    try:
        ...
        return f"<div class='global-card'>{name}<br><span style='font-size:26px; color:{color};'>{curr:,.2f} ({pct:+.2f}%)</span></div>"
    except: return f"<div class='global-card'>{name}<br>장부 확인 불가</div>"

g1.markdown(get_global("^IXIC", "나스닥 (NASDAQ)"), unsafe_allow_html=True)
g2.markdown(get_global("^GSPC", "S&P 500"), unsafe_allow_html=True)
g3.markdown(get_global("^TNX", "미국채 10년 금리"), unsafe_allow_html=True)
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # 1. 종목 헤더 및 거래량
            st.markdown(f"<div class='stock-header'><p style='font-size:35px; color:#1565C0; margin:0;'>{name} ({symbol})</p><p style='font-size:40px; color:#D32F2F; margin:0;'>{format(p, fmt)} {currency}</p></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"""
                ...
                <p style='font-size:20px;'>● <b>필살 지침:</b> 가짜 반등에 속지 마시게. 지표가 바닥 심해로 잠수할 때까지 독하게 기다려야 하네.</p>
                </div>
                """, unsafe_allow_html=True)
...
st.write("---")
st.caption("분석가 서강윤: 2026년 실시간 장부 및 v36056 최종 양식(미장분석+성벽복구) 적용")
