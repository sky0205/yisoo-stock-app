...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [수선] 오늘 가격 제외한 순수 과거 1년 최고가(3,290원 등)를 찾음
            ...
            # [수선] 진짜 전고점(3,290원)을 넘었을 때만 역사적 신고가라고 인정함
            if p > past_high: sig, col, adv = "🚀 역사적 신고가", "#D32F2F", f"● 진짜 1년 최고가({format(past_high, fmt_p)})를 넘었구먼! 즐기되 익절가 잡으시게."
            elif p >= past_high * 0.95: sig, col, adv = "⚔️ 성벽 돌파중", "#E65100", f"● 신고가는 무슨! 전고점({format(past_high, fmt_p)}) 성벽이 안 보이나? 정신 차리시게."
            elif p <= low_b or rsi_val <= 35: sig, col, adv = "🔴 매수권 진입", "#D32F2F", "● 바닥권일세. 분할 매수 보따리 푸시게."
            else: sig, col, adv = "🟡 관망 및 대기", "#FBC02D", f"● 아직 전고점({format(past_high, fmt_p)}) 밑일세. 헛꿈 꾸지 말고 기다리시게."
            ...
            # 네 기둥 지수 상세 진단 (현실 밀착형 훈수)
            i1, i2, i3, i4 = st.columns(4)
            with i1: st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger</p><p class='ind-status'>{'📈 상' if p > mid_line else '📉 하'}</p><p class='ind-diag'>현재 중앙선({format(mid_line, fmt_p)}) 기준 기세를 판독 중일세.</p></div>", unsafe_allow_html=True)
            with i2: st.markdown(f"<div class='ind-box'><p class='ind-title'>RSI (온도)</p><p style='font-size:40px; color:#E65100;'>{rsi_val:.2f}</p><p class='ind-diag'>● **{'👺 불지옥' if rsi_val > 65 else '🧊 냉골' if rsi_val < 35 else '미지근'}** 상태일세. 냉정하게 보시게.</p></div>", unsafe_allow_html=True)
