...
def fetch_kr_orderbook(symbol):
    return {
        "ask": 0.0, "bid": 0.0, "ratio": None, "ok": False, "msg": "실시간 호가 API 미연결",
    }
...
st.markdown(
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    div.stButton > button {
        ...
        padding: 10px 15px !important; height: 46px !important; border-radius: 8px !important;
        border: 2px solid #FFEB3B !important; width: 100% !important; cursor: pointer !important;
    }
    </style>
    """,
...
st.title("🧐 이수할아버지의 냉정 진단기 v36078 (4차 무결점 보정판)")
display_global_risk()
st.divider()

col_symbol, col_manual, col_avg, col_ask, col_bid, col_btn = st.columns([1.5, 1.4, 1.4, 1.4, 1.4, 1.0])
...
with col_manual:
    manual_price_str = st.text_input("⚡ 수동 실시간가 (선택)", value="").strip()

with col_avg:
    user_avg_price = st.number_input("💡 보유 평단가", min_value=0.0, value=0.0, step=100.0)

with col_ask:
    manual_ask = st.number_input("🔴 HTS 총매도잔량", min_value=0.0, value=0.0, step=1.0, format="%.2f")

with col_bid:
    manual_bid = st.number_input("🔵 HTS 총매수잔량", min_value=0.0, value=0.0, step=1.0, format="%.2f")
...
if symbol:
    try:
        ...
        if is_kr:
            ...
            # ★ [지적 반영 4]: 크롤링 실패 시 st.toast 경고 안전망 추가
            if not kr_fetched:
                try:
                    ...
                    st.toast("⚠️ 실시간 시세 미확인: 네이버 크롤링 실패로 캐시 데이터 또는 차트 종가로 대체합니다.", icon="🚨")
                    if not df.empty:
        ...
        if manual_ask > 0 and manual_bid > 0:
            ...
            ob_data = {"ask": manual_ask * 1000.0, "bid": manual_bid * 1000.0, "ratio": calc_ratio, "ok": True}
        ...
        if df.empty:
            st.warning(f"⚠️ [{symbol}] 종목 데이터를 불러오지 못했구먼.")
        else:
            df = df.ffill().dropna()
            ...
            if not is_kr and us_prev_p and us_prev_p > 0:
                ...
                try:
                    ...
                    df_past = df_sorted.drop(today_date, errors="ignore") if today_date in df_sorted.index else df_sorted
                    prev_p = float(df_past["Close"].iloc[-1]) if len(df_past) >= 1 else p
                except Exception:
                    prev_p = float(df["Close"].iloc[-2]) if len(df) >= 2 else p

            # ★ [지적 반영 2]: 보합(변동 없음) 왜곡 조작 로직 완전 박멸 (정직한 비교)
            ...
            # ★ [지적 반영 3]: 날별 하단 밴드 시리즈 구축 (recent_bottom_memory 정확도 확보)
            ...
            is_orderbook_safe = float(ob_ratio_val) <= 1.5 if ob_ratio_val is not None else False
            ...
            # 종목명 복원
            if is_kr:
                core_vault = {"101490": "에스앤에스텍", "005930": "삼성전자", "000660": "SK하이닉스"}
                final_display_name = core_vault.get(clean_symbol, f"국내종목 ({symbol})")
            else:
                final_display_name = f"{symbol.upper()}"

            safe_display_name = html.escape(final_display_name)

            # 신호등 우선순위 판정
            is_stop_loss_triggered = (user_avg_price > 0 and p < ma5_val * 0.95) or (recent_bottom_memory and p < float(df["Low"].iloc[-61:-1].min() if len(df) > 60 else df["Low"].min()))
            ...
            elif p >= up_b * 0.995 and bandwidth >= 15.0 and is_ma5_safe:
                final_code = "BAND_RIDING_HARVEST"
                sig = "🟣 [밴드 라이딩 대시세] 목표선 상방 확장 중!"
                col = "#6A1B9A"
                final_adv = "• <b>[최종 결론]</b> 상단 확장 중! 50% 익절 후 5일선 기준으로 잔여 추종하시게."
            elif p >= up_b * 0.995:
                final_code = "RED_SELL_TARGET"
                sig = "🔴 [목표 도달] 수학 목표선 저항! 1차 50% 수확!"
                col = "#D32F2F"
                final_adv = "• <b>[최종 결론]</b> 상단 저항 도달 완료! 분할 현금화를 집행하시게."
            elif not is_orderbook_safe:
                final_code = "WAIT_ORDERBOOK"
                sig = "🟡 [관망/보류] 실시간 호가 미연결 또는 매도벽 우세"
                col = "#F57C00"
                final_adv = "• <b>[최종 결론]</b> 호가 검증 데이터가 부족하거나 매도벽이 두터우니 관망하시게."
            else:
                final_code = "WAIT_GENERAL"
                sig = "🟡 [관망] 조건 미충족 / 뇌동매매 금지"
                col = "#FBC02D"
                final_adv = "• <b>[최종 결론]</b> 명확한 주도 신호가 충족되지 않았으므로 관망을 유지하시게."
            ...
            st.success("👵 4차 핵심 모순 수정이 완벽하게 반영되었사옵니다, 할배!")
