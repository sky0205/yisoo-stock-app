...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [집중 수복] 4대 지수 상세 설명 (image_abbc54.png 기반 100% 복구)
            st.divider()
            i1, i2, i3, i4 = st.columns(4)
            with i1: # Bollinger
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p class='ind-diag'>● **[상단 돌파!]** 하늘 찌르네! 익절 준비 하시게. 평균 회귀 본능에 의해 조만간 소나기가 올 수 있으니 보따리 싸시게.</p></div>", unsafe_allow_html=True)
            with i2: # RSI
            ...
            with i4: # MACD
                st.markdown(f"<div class='ind-box'><p class='ind-title'>MACD (엔진)</p><p class='ind-diag'>● {'엔진 정회전! 기세 좋구먼.' if m_l > s_l else '엔진 **역회전** 중! 절대 속지 마시게.'} 폭풍우가 지나가길 기다리는 게 상책일세.</p></div>", unsafe_allow_html=True)
