...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            high_365 = float(df.iloc[:-1]['Close'].max()) # 오늘 제외 과거 최고가
            ...
            # [복구] 신호등 단순화 (매수/매도/관망)
            if p >= up_b or rsi_val >= 65: sig, col, adv = "🟢 매도권 진입", "#388E3C", f"● 과열권일세! 전고점({format(high_365, fmt_p)}) 근처니 수익 챙기시게."
