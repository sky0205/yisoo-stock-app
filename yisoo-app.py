...
def fetch_ai_stock_info(user_input):
    try:
        ...
        # AI 적정주가 계산 (마이너스 방지 로직)
        info = stock.info
        bps = info.get('bookValue') or 0
        roe = info.get('returnOnEquity') or 0.10
        r = 0.09 
        
        # BPS가 마이너스면 적정주가 산출 불가(0으로 표시)
        if bps <= 0:
            target_val = 0.0
        else:
            target_val = bps + (bps * (roe - r) / r)
            if target_val < 0: target_val = bps * 0.5 # 극단적 저평가 시 BPS 절반이라도 표시
...
if ai_data:
    ...
    if st.button("🚀 실시간 AI 정밀 분석 시작"):
        ...
        if not df.empty:
            ...
            st.markdown(f"<div class='target-box'>💎 적정주가 기준: {f_tg if in_target > 0 else '산출 불가'}</div>", unsafe_allow_html=True)

            # 요약
            st.markdown("### 📝 AI 추세 분석 요약")
            sum_msg = "상승세가 강해 상단을 돌파 중입니다." if price > up_band else "바닥을 다지는 중입니다."
            st.markdown(f"<div class='summary-box'><b>의견:</b> 현재 {in_name}은(는) {sum_msg}<br>RSI {round(rsi,1)}는 {'과열' if rsi>70 else '바닥' if rsi<35 else '안정'}입니다.</div>", unsafe_allow_html=True)
            ...
            st.table(pd.DataFrame({
                ...
                "AI 진단": ["상단 돌파" if price > up_band else "하단 지지" if price < dn_band else "정상", "과열" if rsi>70 else "바닥" if rsi<30 else "보통", "단기천장" if wr>-20 else "단기바닥" if wr<-80 else "보통", "상승세" if macd_val>0 else "하락세"]
            }))
        else:
            st.error("데이터 로딩 실패!")
else:
