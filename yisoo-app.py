...
# 글로벌 지수 판독 (에러 방지용 캐시 적용)
@st.cache_data(ttl=300)
def get_global_data():
    indices = {"^IXIC": "나스닥 (NASDAQ)", "^GSPC": "S&P 500", "^TNX": "미 국채 10년"}
    data = {}
...
def display_global_risk():
    try:
        ...
        if "나스닥 (NASDAQ)" in g_data:
            val, chg = g_data["나스닥 (NASDAQ)"]
            c1.metric("나스닥 (NASDAQ)", f"{val:,.0f}", f"{chg:.2f}%")
            if "S&P 500" in g_data:
                v2, c2_val = g_data["S&P 500"]
                c2.metric("S&P 500", f"{v2:,.0f}", f"{c2_val:.2f}%")
            if "미 국채 10년" in g_data:
                v3, c3_val = g_data["미 국채 10년"]
                c3.metric("미 국채 10년", f"{v3:.2f}%", f"{c3_val:+.2f}%")
            
            advice = f"✅ **[미장 쾌청: 진격!]**" if chg > 0.5 else "🚨 **[긴급 상황: 정박!]**" if chg < -1.0 else "🧐 **[안개 정국: 관망]**"
            st.info(f"🧐 이수 할배의 글로벌 판독: {advice} 지표 끝단을 기다리시게.")
        st.markdown("</div>", unsafe_allow_html=True)
    except: st.error("⚠️ 글로벌 데이터 호출 불가 (잠시 후 다시 시도하십시오)")
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            if chg_rate > 2.0 and v_ratio < 100: v_adv = "🚨 <b>가짜 상승!</b> 빈집에 바람 드는 격이니 절대 속지 마십시오."
            else: v_adv = "✅ 세력의 발자국을 추적 중일세."
            ...
            with i1: st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>Bollinger (기세)</p><p class='ind-diag'>● **[상단 돌파!]** 하늘 찌르네! 익절 준비 하시게. 소나기 올 수 있으니 보따리 싸시게.</p></div>", unsafe_allow_html=True)
            with i2: st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>RSI (온도)</p><p style='font-size:40px; color:#E65100;'>{rsi_val:.1f}</p><p class='ind-diag'>● 온도 {rsi_val:.2f}로 **탐욕의 불지옥** 직전일세!</p></div>", unsafe_allow_html=True)
            with i3:
                h14 = df['High'].rolling(14).max().iloc[-1]; l14 = df['Low'].rolling(14).min().iloc[-1]; will_val = (h14 - p) / (h14 - l14 + 1e-10) * -100
                st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>Williams %R</p><p style='font-size:40px; color:#E65100;'>{will_val:.1f}</p><p class='ind-diag'>● 지수 {will_val:.1f}로 **항복 지점** 근처일세!</p></div>", unsafe_allow_html=True)
            with i4: st.markdown(f"<div class='ind-box'><p style='font-size:26px; color:#1976D2;'>MACD (엔진)</p><p class='ind-diag'>● {'엔진 정회전!' if m_l > 0 else '엔진 **역회전** 중!'}</p></div>", unsafe_allow_html=True)

    except Exception as e: st.error(f"👵 아이구! 오류: {e} (데이터 호출 제한 가능성)")
