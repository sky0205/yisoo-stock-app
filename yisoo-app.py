...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            with i1: # Bollinger
                bb_txt = "⚠️ 하단 성벽 돌파! 실리콘투처럼 하단 근처면 투매 비명이 그칠 때까지 기다려야 해." if p <= low_b * 1.01 else "⚖️ 중앙선 아래서 빌빌대네. 에너지가 아래로 흐르고 있어." if p < mid_line else "📈 중앙선 위에서 기세 타네."
