...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    .main-title { font-size: 65px !important; color: #FFFFFF !important; margin: 0; }
    ...
    .global-header { font-size: 28px !important; color: #81D4FA !important; border-bottom: 2px solid #546E7A; padding-bottom: 15px; margin-bottom: 20px; }
    ...
    .strategy-title { font-size: 36px !important; color: #D32F2F !important; border-bottom: 4px solid #FFEBEE; padding-bottom: 15px; margin-bottom: 25px; }
    .diagnosis-content { 
        font-size: 25px !important; color: #B71C1C !important; line-height: 1.8; 
        background-color: #FFF8F1; padding: 25px; border-radius: 12px; 
        border-left: 12px solid #D32F2F; margin-bottom: 25px; 
    }
    
    /* 지표 박스 및 수치 제목 */
    .ind-box { background-color: #FFFFFF; padding: 30px; border-radius: 15px; border: 3px solid #90A4AE; min-height: 480px; }
    ...
    .value-label { font-size: 22px !important; color: #546E7A !important; text-align: center; display: block; margin-top: 10px; }
    .ind-value { font-size: 65px !important; color: #B71C1C !important; text-align: center; display: block; margin-bottom: 15px; }
...
st.markdown(f"""
    <div class='global-unified-box'>
        ...
        <div style='display: flex; justify-content: space-around; align-items: center;'>
            ...
            <div class='global-item-text'>S&P 500: <span style='color:{"#EF5350" if nas_pct > 0 else "#42A5F5"};'>{sp_val:,.2f} ({nas_pct:+.2f}%)</span></div>
        ...
        <div style='font-size:20px; color:#CFD8DC;'>
            <b>👴 냉정 평가:</b> 미장 거품은 여전히 위태롭고, 국채 금리는 언제든 비수가 될 수 있네. 국장은 사지가 묶인 격이니 절대 섣부른 낙관은 금물일세.
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            with i1: st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ Bollinger Bands</p><span class='value-label'>[중앙선 가격]</span><span class='ind-value'>{format(mid_line, fmt)}</span><p style='font-size:20px;'>● 중앙선 {'위에서 기세는 있으나' if p > mid_line else '아래로 꺾여 기운이 없구먼.'} 상단 성벽 돌파 여부를 보게나.</p></div>", unsafe_allow_html=True)
            with i2: st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ RSI (상대강도)</p><span class='value-label'>[현재 RSI 지수]</span><span class='ind-value'>{rsi_val:.1f}</span><p style='font-size:20px;'>● 현재 수치 <b>{rsi_val:.1f}</b>일세. 30 이하로 식을 때까지 총을 아끼게.</p></div>", unsafe_allow_html=True)
            with i3: st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ Williams %R</p><span class='value-label'>[현재 윌리엄 지수]</span><span class='ind-value'>{will_val:.1f}</span><p style='font-size:20px;'>● 수치 <b>{will_val:.1f}</b>는 개미들의 항복 지점을 보여주네. -80 아래가 진짜 바닥일세.</p></div>", unsafe_allow_html=True)
            with i4: st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ MACD (추세엔진)</p><span class='value-label'>[엔진 상태]</span><span class='ind-value' style='font-size:50px !important;'>{'▲ 상승' if m_l > s_l else '▼ 하락'}</span><p style='font-size:20px;'>● 엔진이 {'정방향' if m_l > s_l else '역회전'} 중이야. 맥점을 독하게 째려보게나.</p></div>", unsafe_allow_html=True)

    except Exception as e: st.error(f"장부를 불러올 수 없구먼 (오류: {e})")

st.write("---")
st.caption("분석가 서강윤: 2026년 실시간 장부 및 v36056 최종 보완본 적용 완료")
