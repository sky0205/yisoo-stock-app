...
if symbol:
    try:
        ...
        if not df.empty:
            df = df.ffill().dropna()
            # [현재가 사수] yfinance fast_info가 있다면 실시간 가격을 우선 취득
            try:
            ...
            m_l = macd.iloc[-1]; s_l = signal.iloc[-1]; m_p = macd.iloc[-2]; s_p = signal.iloc[-2]
            ...
            if v_ratio >= 30 and is_opening:
                if p_chg >= 3: v_status, v_adv = f"🔥 시초 폭등 / 거래폭발", f"🔥 **[세력 진격!]** 거래량이 {v_ratio:.1f}% 터지며 폭등 중일세! 빳빳하게 기세 타시게."
                elif p_chg <= -3: v_status, v_adv = f"💀 시초 폭락 / 거래폭발", f"💀 **[비명 포착!]** 거래량이 {v_ratio:.1f}% 터지며 폭락 중일세! 일단 피신하시게."
                else: v_status, v_adv = f"📈 시초 거래급등", f"✅ 거래량 {v_ratio:.1f}%로 터졌으나 힘겨루기 중일세."
            else:
                v_status = f"{v_label} ({v_ratio:.1f}%)"
                v_adv = f"🔥 **[진짜 상승!]** 거래량 실린 빳빳한 진격일세!" if p_chg > 3 and v_ratio > 150 else f"✅ 현재 5일 평균 대비 거래율 {v_ratio:.1f}%로 세력을 추적 중일세."
            ...
            elif p <= (low_b * 1.005) or rsi_val <= 35: sig, col, s_adv = "🔴 매수권 진입", "#D32F2F", "● 🧊 바닥권일세. 보따리 푸시게."
            ...
            if p >= up_b or rsi_val >= 60: final_adv = f"💰 **[최종 결론]** 거래강도({vol_strength:.0f}점). 탐욕의 끝자락일세. **분할 매도**하여 수익을 챙기시게!"
                ...
                tag = "🔥" if vol_strength > 150 else "🛡️"
            ...
            with i1:
                bb_diag = "⚠️ **[과열 진입]** 온도가 높네. 익절 준비하시게." if p >= up_b or rsi_val >= 60 else ("🏰 **[성벽 사수]** 안정적 진격 중일세." if p > mid_line else "🏚️ **[성문 함락]** 절대 칼 뽑지 마시게.")
            ...
            with i2:
                ...
                r_diag = f"● 지수 {rsi_val:.2f}로 **{'👺 불지옥' if rsi_val >= 60 else '🧊 냉골' if rsi_val <= 35 else '중립'}**일세. {'🚨 가짜 상승 주의!' if is_div else ''}"
            ...
            with i3:
                w_diag = f"● 지수 {will_val:.2f}로 **{'🧨 천장 광기' if will_val >= -20 else '📉 하락 가속' if will_val <= -65 else '중간 지대'}**일세."
            ...
            with i4:
                ...
                m_diag = "● 엔진 **정회전**!" if m_l > s_l else ("● 엔진 **역회전폭 급감**! 시동 준비!" if m_diff > m_diff_prev else "● 엔진 **역회전 심화**! 자숙하시게.")
