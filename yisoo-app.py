...
def display_global_risk():
    ...
    try:
        ...
        # [상세 훈수 로직]
        if n_chg < -1.0 or tnx_val > 4.5:
            advice = f"🚨 **[긴급: 정박하시게!]** 나스닥 폭락 중이고 국채 금리({tnx_val:.2f}%)가 치솟으니 시장이 발작하고 있구먼. 성벽 다 무너지기 전에 보따리 싸서 피신해 계시게!"
        elif n_chg > 0.8 and tnx_val < 4.0:
            advice = f"✅ **[쾌청: 진격하시게!]** 미장이 불을 뿜고 금리도 안정세일세. 세력이 돈 풀고 있으니 빳빳하게 기세 타고 올라갈 종목 골라보시게."
        else:
            advice = f"🧐 **[안개: 낚싯대만 던지시게]** 지수가 갈피를 못 잡고 눈치만 보고 있구먼. 무리하게 진격하지 말고 지표 바닥권 종목에 낚싯대만 하나 던져두고 기다리시게."
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            with i1: # Bollinger
                bb_diag = f"● 현재 **하단 성벽({format(low_b, fmt_p)})** 터치 중일세! 여기서 지지받고 거래량 실리면 진입을 노려볼 명당자리구먼!" if p <= low_b else f"● 현재 중앙선 아래일세. 하단 성벽까지 기다렸다가 지지 확인하고 낚싯대 던지시게."
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p class='ind-status'>{'📉 하락/바닥권' if p < mid_line else '📈 상승'}</p><p class='ind-diag'>{bb_diag}</p></div>", unsafe_allow_html=True)
            with i2: # RSI
                st.markdown(f"<div class='ind-box'><p class='ind-title'>RSI (온도)</p><p style='font-size:40px; color:#E65100;'>{rsi_val:.2f}</p><p class='ind-diag'>● **{'🧊 냉골' if rsi_val < 35 else '👺 불지옥' if rsi_val > 65 else '미지근'}** 상태일세. 남들 환호할 때 냉정하게 보시게.</p></div>", unsafe_allow_html=True)
            with i3: # Williams
            ...
            with i4: # MACD
                st.markdown(f"<div class='ind-box'><p class='ind-title'>MACD (엔진)</p><p class='ind-status'>{'▲ 상승' if m_l > s_l else '▼ 하락'}</p><p class='ind-diag'>● 추세의 엔진이 어디로 도는지 보시게. 함부로 칼날 잡지 마시게.</p></div>", unsafe_allow_html=True)
