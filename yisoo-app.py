...
if symbol:
    try:
        # [핵심] 과거 1년치 데이터로 진짜 전고점 판독 (오늘 제외)
        ...
        if not df.empty:
            ...
            # [수선] 진짜 전고점 판독: 오늘 데이터를 제외한 과거 최고가(흥아해운 3,290원 기준)
            high_365 = float(df.iloc[:-1]['Close'].max()) 
            ...
            v_adv = "🚨 가짜 상승 주의!" if p > prev_p and v_ratio < 100 else "✅ 관심 집중!"
            ...
            # [핵심 수선] 신호등 로직 (진짜 전고점 3,290원 넘기 전까진 빨간불 금지)
            if p > high_365: sig, col, adv = "🚀 역사적 신고가", "#D32F2F", f"● 진짜 1년 최고가({format(high_365, fmt_p)})를 넘었구먼! 즐기되 익절가 잡으시게."
            elif p >= high_365 * 0.95: sig, col, adv = "⚔️ 성벽 돌파중", "#E65100", f"● 신고가는 무슨! 전고점({format(high_365, fmt_p)}) 성벽이 안 보이나? 정신 차리시게."
            elif p <= low_b or rsi_val <= 35: sig, col, adv = "🔴 매수권 진입", "#D32F2F", "● 바닥권일세. 분할 매수 보따리 푸시게."
            else: sig, col, adv = "🟡 관망 및 대기", "#FBC02D", f"● 아직 전고점({format(high_365, fmt_p)}) 밑일세. 낚싯대만 던져두시게."
            ...
            # [복구] 네 기둥 지수 상세 진단 (현실적 훈수)
            i1, i2, i3, i4 = st.columns(4)
            with i1: st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p class='ind-status'>{'📈 상' if p > mid_line else '📉 하'}</p><p class='ind-diag'>현재 중앙선({format(mid_line, fmt_p)}) 기준 기세를 보시게.</p></div>", unsafe_allow_html=True)
