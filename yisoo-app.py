...
# 1. 화면 구성 및 디자인 (모바일 최적화 및 지수 설명 칸 강화)
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            if p > prev_p: v_advice = "📈 <b>주포의 진격:</b> 거래량이 실린 상승일세. 외인/기관이 벽을 뚫고 있으니 기세가 살아있구먼." if v_ratio > 150 else "🚨 <b>가짜 축제:</b> 거래량 없이 가격만 올렸어. 개미 꼬시는 함정이니 절대 쫓지 마셔." if v_ratio < 80 else "⏳ <b>간보기 상승:</b> 적당한 거래량으로 눈치 보는 중이야. 추격보다는 성벽을 보게나."
            else: v_advice = "⚠️ <b>도살장 입구:</b> 거래량 터진 하락이야. 주포들이 보따리 쌌으니 지하실 구경하기 싫으면 엎드리셔." if v_ratio > 150 else "💤 <b>무관심 하락:</b> 파는 놈도 사는 놈도 없어. 매수세가 마른 형국이니 바닥 성벽만 째려보게나." if v_ratio < 80 else "📉 <b>계단식 하락:</b> 야금야금 물량이 쏟아지고 있구먼. 급할 것 없으니 심해가 나올 때까지 기다리셔."
            ...
            st.markdown(f"""
                <div class='unified-strategy-box'>
                    ...
                    <div style='font-size:16px; color:#333;'>● <b>지침:</b> 가짜 반등에 속지 마시게. 지표가 바닥 심해로 잠수할 때까지 독하게 기다려야 하네.</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with i4: # MACD
                ...
                st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ MACD (추세)</p><span style='font-size:13px; color:#546E7A;'>[엔진 상태]</span><span class='ind-value' style='font-size:26px !important;'>{'▲ 상승' if m_l > s_l else '▼ 하락'}</span><div class='ind-diag'>● <b>판독:</b> {m_txt}<br>엔진이 역회전(데드크로스) 중일 때는 아무리 좋은 기사가 나도 믿지 마셔. 추세 마디가 바뀌고 시그널선 위로 올라타야 비로소 배가 움직이는 법일세.</div></div>", unsafe_allow_html=True)
