...
def display_global_risk():
    ...
    try:
        ...
        else: advice = "🧐 **[안개 속: 관망]** 눈을 부라리고 보시게."
        ...
        if not df.empty:
            ...
            if v_ratio >= 30 and is_opening:
                if p_chg >= 3:
                    ...
                    v_adv = f"🔥 **[세력 진격!]** 시초에 거래량이 5일 평균 대비 {v_ratio:.1f}% 터지며 주가 폭등 중일세! 빳빳하게 기세 타시게!"
                elif p_chg <= -3:
                    v_status = f"💀 현지 시초장 주가 폭락 / 거래 폭발 ({v_ratio:.1f}%)"
                    v_adv = f"💀 **[비명 포착!]** 시초에 거래량이 {v_ratio:.1f}% 터지며 폭락 중일세! 성벽 함락 중이니 일단 피신하시게!"
                else:
            ...
            st.markdown(f"""<div class='trend-card'><div class='trend-title'>⚔️ {name} 실전 필살 대응 전략</div>
                ...
                <div class='trend-item'>● <b>필살 조언:</b> <span style='color:#D32F2F;'>{'과열권이니 수익 챙기시게!' if p >= up_b else '바닥 확인하고 진격하시게!'}</span></div></div>""", unsafe_allow_html=True)

            # [핵심 수선] 네 기둥 지수 상세 훈수 (볼린저 상단 판독 수정)
            st.divider()
            i1, i2, i3, i4 = st.columns(4)
            with i1: # Bollinger
                if p >= up_b:
                    bb_diag = f"● **[비상: 상단 성벽 돌파!]** 가격이 상단 성벽({format(up_b, fmt_p)})을 뚫고 하늘을 찌르는구먼! 탐욕에 눈이 멀지 말고 이제는 수확(익절) 준비를 빳빳하게 하시게."
                elif p <= low_b:
                    bb_diag = f"● **[비상: 하단 성벽 돌파!]** 하단 성벽 아래일세! 여기서 지지받고 고개 들면 진짜 90% 승률의 진격 기회일세."
                elif p < mid_line:
                    bb_diag = f"● 현재 중앙선 아래서 빌빌대고 있구먼. 성벽 사수 확인 전까지는 낚싯대만 던지시게."
                else:
                    bb_diag = f"● 현재 중앙선 위에서 상단 성벽을 향해 기세 좋게 가는 중일세. 상단 돌파 여부를 매섭게 보시게."
            ...
            with i2: # RSI
                r_diag = f"● 지수 {rsi_val:.2f}로 **🧊 냉골(과매도)** 상태일세! 남들 무서워서 던질 때 우리는 냉정하게 바닥을 보시게." if rsi_val < 35 else f"● 지수 {rsi_val:.2f}로 **👺 불지옥(과열)** 구간일세! 탐욕에 눈멀면 비수 꽂히네. 익절가 잡으시게." if rsi_val > 65 else f"● 현재 중립 기어 넣고 눈치싸움 중일세."
            ...
            with i3: # Williams
                w_diag = f"● 지수 {will_val:.2f}로 **🏳️ 개미 항복 구간**일세! 보따리 푸시게. 여기서 고개 들면 무조건 진격일세!" if will_val < -80 else f"● 지수 {will_val:.2f}로 **🧨 천장 광기 구간**일세! 언제 비수가 꽂힐지 모르니 매섭게 보시게." if will_val > -20 else f"● 현재 중간지대일세. 바닥인지 천장인지 갈피 못 잡고 있구먼."
