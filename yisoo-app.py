    ...
    try:
        ...
        if not df.empty:
            ...
            with i4: # MACD (엔진) 상세 진단
                ...
                m_diag = "● 엔진이 **정회전** 중일세! 기세 붙었으니 진격 보시게." if m_l > s_l else "● 엔진이 **역회전** 중이네! 거꾸로 도는 차에 올라타면 안 되는 법일세."
                
                st.markdown(f"""
                    <div class='ind-box'>
                        <p class='ind-title'>MACD (엔진)</p>
                        <p class='ind-val' style='color:{m_color};'>MACD {m_l:,.2f} / Signal {s_l:,.2f} ({m_status})</p>
                        <p class='ind-diag'>{m_diag}</p>
                    </div>
                """, unsafe_allow_html=True)

            # 윌리엄 지수 진단 (울타리 밖으로 빼내서 깔끔하게 정리했네)
            if will_val <= -80:
                w_diag = f"● 지수 {will_val:.2f}로 **🏳️ 개미 항복** 구간일세! 바닥 찍었으니 고개 들면 무조건 진격일세."
            elif will_val <= -60:
                w_diag = f"● 지수 {will_val:.2f}로 **📉 하단 진흙탕**에서 헤매는 중일세! 기어 나올 힘이 없으니 함부로 보따리 풀지 마시게."
