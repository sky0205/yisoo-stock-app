...
# 1. 화면 구성 및 디자인 (v36056 정통 스타일 복구)
...
st.title("👴 이수할아버지의 냉정 진단기 v36056 (성벽 복구본)")

# 2. 글로벌 전황 보고 (수치만 깔끔하게 표시)
...
def get_global(ticker, name):
    ...
    except: return f"<div class='global-card'>{name}<br>연결 실패</div>"
...
g3.markdown(get_global("^TNX", "US 10Y (미국채 금리)"), unsafe_allow_html=True)
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            v_advice = "🚨 <b>가짜 상승!</b> 주인(외인)은 나가는데 머슴(기관)이 억지로 막고 있구먼. 속지 마셔." if (p > prev_p and v_ratio < 100) else "⚠️ <b>도살장 전조.</b> 거래량이 실린 하락이야. 일단 대피하셔." if (p < prev_p and v_ratio > 150) else "⏳ 안개 속이니 지켜만 보셔."
            ...
            with c1: st.markdown(f"<div class='price-card'><p>⚖️ 공략 대기선(하단)</p><p style='font-size:32px; color:#388E3C;'>{format(low_b, fmt)}</p></div>", unsafe_allow_html=True)
            with c2: st.markdown(f"<div class='price-card'><p>🎯 수확 목표선(상단)</p><p style='font-size:32px; color:#D32F2F;'>{format(up_b, fmt)}</p></div>", unsafe_allow_html=True)
            with c3: st.markdown(f"<div class='price-card'><p>🛡️ 방어 성벽 (생명선)</p><p style='font-size:32px; color:#E65100;'>{format(defense_line, fmt)}</p></div>", unsafe_allow_html=True)

            # 4. 네 기둥 지수 상세 진단
            st.subheader("🏗️ 4. 네 기둥 지수 상세 진단 (Four Pillars)")
            ...
            with i3: # Williams %R x 지표
                w_diag = f"지수 {will_val:.1f}. 신용 잔고 33조 원은 시한폭탄일세. 윌리엄 지수가 -90 근처에서 횡보하며 모든 개미가 항복을 선언할 때 진짜 기회가 숨어 있구먼."
...
st.write("---")
st.caption("분석가 서강윤: 2026년 실시간 장부 및 v36056 최종 양식(성벽 복구본) 적용 완료")
