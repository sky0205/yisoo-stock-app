...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    .global-header { font-size: 18px !important; color: #81D4FA !important; border-bottom: 1px solid #546E7A; padding-bottom: 8px; margin-bottom: 12px; }
    ...
    /* 4. 네 기둥 지수 상세 진단 */
    .ind-box { background-color: #FFFFFF; padding: 15px; border-radius: 12px; border: 2px solid #90A4AE; min-height: 380px; margin-bottom: 10px; }
    ...
    .ind-value { font-size: 35px !important; color: #B71C1C !important; text-align: center; display: block; margin: 8px 0; }
...
st.markdown(f"""
    <div class='global-unified-box'>
        ...
        <div style='font-size:14px; color:#CFD8DC;'><b>👴 냉정 평가:</b> 미장 거품 위태롭고 금리는 비수일세. 국장은 사지 묶인 격이니 섣부른 낙관 마셔.</div>
    </div>
    """, unsafe_allow_html=True)
        ...
        if not df.empty:
            ...
            st.markdown(f"""
                <div class='volume-box-unified'>
                    ...
                    <div style='font-size:15px;'>{'🚨 가짜 상승! 속지 마셔.' if p > prev_p and v_ratio < 100 else '⚠️ 물량 쏟아지니 성벽 보셔.' if p < prev_p and v_ratio > 150 else '⏳ 대기하셔.'}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with i4: # MACD
                st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ MACD (엔진)</p><span style='font-size:13px; color:#546E7A;'>[엔진 상태]</span><span class='ind-value' style='font-size:28px !important;'>{'▲ 상승' if m_l > s_l else '▼ 하락'}</span><div class='ind-diag'>● 엔진 {'정방향' if m_l > s_l else '역회전'} 중일세. 엔진이 멈추고 방향을 틀 때까지는 아무리 좋은 소식이 들려도 장부를 믿으셔.</div></div>", unsafe_allow_html=True)
