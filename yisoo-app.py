        ...
        if df.empty:
            ...
            if bandwidth < 12.0:
                ...
                if p >= ma5_val:
                    ...
                    is_bandwidth_ok = not is_bearish_zone
                else:
                    is_bandwidth_ok = False
            else:
            ...
            if is_stop_loss_triggered:
                ...
                col = "#6A1B9A"
                final_adv = "• <b>[최종 결론]</b> 볼린저 상단 확장 중! <b>물량의 50%는 익절</b>하고, <b>잔여 50%는 5일선 이탈 전까지 홀딩</b>하시게."
            elif is_target_reached:
                sig = "🔴 [목표 도달] 수학 목표선 저항! 1차 50% 분할 매도!"
                col = "#D32F2F"
                final_adv = "• <b>[최종 결론]</b> 볼린저 상단 저항 도달! <b>우선 50% 물량을 기계적으로 수확</b>하시게."
            elif not is_orderbook_safe:
                sig = "🟡 [관망/보류] 실시간 호가 미연결 또는 매도벽 우세"
                col = "#F57C00"
                final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f})점. 호가 검증 데이터가 부족하거나 매도벽이 두터우니 관망하시게."
            elif is_bottom_entry_signal or is_escape_buy_signal:
                sig = f"🟢 [진바닥 탈출/안착] 매수 진격 유효 구역 ({time_tag_ok})"
                col = "#2E7D32"
                final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f})점. <b>[지지 안착]</b> {time_rule_desc}"
            elif vol_strength < 80 and not (first_entry_done or p_chg >= 0):
                sig = "🟡 [거래절벽 관망] 수급 마름 / 섣부른 진입 금지"
                col = "#F57C00"
                final_adv = f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f})점. 수급이 마르고 동력이 없으니 관망하시게."
            else:
            ...
            st.success("👵 머리부터 꼬리까지 잘림 없이 온전히 엮인 v36075 완결판이옵니다, 할배!")
