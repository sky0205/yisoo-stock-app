...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    .ind-box { background-color: #FFFFFF; padding: 18px; border-radius: 12px; border: 2px solid #90A4AE; min-height: 420px; margin-bottom: 15px; }
    ...
    .ind-diag { font-size: 14px !important; color: #333333 !important; line-height: 1.7; background-color: #FDFDFD; padding: 10px; border-radius: 5px; border-left: 6px solid #D32F2F; }
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # 2. 필살 전략 + 냉정 진단 (대응 전략 복구 완료)
            if p <= low_b * 1.03: strategy_txt = "<b>하단 성벽 사수:</b> 모든 개미가 투항할 때까지 총을 아끼며 독하게 기다리시게. 지표가 바닥 심해를 뚫을 때가 기회일세."
            elif p >= up_b * 0.97: strategy_txt = "<b>수확의 시간:</b> 욕심이 화를 부르네. 성벽 상단에 닿았으니 보따리 쌀 준비를 하고 수익금을 챙기셔."
            else: strategy_txt = "<b>함정 수사 대기:</b> 가짜 반등에 속아 방아쇠를 당기지 마시게. 지표가 완전히 식을 때까지는 그저 째려만 보셔."
            ...
            st.markdown(f"""
                <div class='unified-strategy-box'>
                    ...
                    <div style='font-size:16px; color:#333; padding-left:10px;'>
                        ● <b>필살 대응 전략:</b> {strategy_txt}
                    </div>
                </div>
                """, unsafe_allow_html=True)
