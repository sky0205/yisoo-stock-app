    ...
    try:
        ...
        if df.empty:
           ...
           # 전일 종가 변수는 위에서 확정한 prev_p를 사용한다.
            ...
            # ==================================================================
            # ★ [신호등 분기 논리 - 혼조세 시 다중 지표 비교 엄격 적용 완성본]
            # ==================================================================
            try:
            ...
            if is_stop_loss_triggered:
            ...
            elif is_ma_tangled and (p < mid_line or bottom_score < 2 or vol_strength < 75.0):
                # 🟡 [혼조세 철저 통제]: 이평선이 엉켜 있는데 20일선 저항 아래에 있거나 진바닥 점수/수급이 부족하면 무조건 관망(노란불)
                ...
                if p < mid_line:
                    final_adv = (
                        f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                        f"<b>[이평선 꼬임 및 20일선 저항]</b> 이평선들이 엉켜 있고 머리 위 20일선({mid_line:{fmt_p}}{currency}) 저항 아래에 갇혀 있으므로, "
                        "지표 점수에 현혹되지 말고 확실한 정배열 분출이 나올 때까지 관망하시게."
                    )
                else:
                    final_adv = (
                        f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
