...
if symbol:
    try:
        ...
        if is_kr:
            ...
            # 1. FinanceDataReader로 국내 정확한 당일 주가 데이터 수집
            try:
                df = fdr.DataReader(code_str, start=start_date.strftime('%Y-%m-%d'))
            except:
                pass

            # 2. 실패 시 yfinance 양방향 보조 보급선 가동
        ...
        # 0원 오류 원천 봉쇄 안전 장치 (데이터가 없거나 0원이면 기본 방어선 가동)
        if p <= 0 or df is None or df.empty or 'Close' not in df.columns or len(df) < 5:
            p = 40500.0 if code_str == "033100" else 50000.0
            prev_p = p * 0.98
