...
# [복구 및 강화] 글로벌 전황: 어르신의 매서운 판독 내용 상세화
def display_global_risk():
    try:
        ...
        if n_chg > 0.5 and t_chg < 0:
            advice = f"✅ **[미장 쾌청: 진격!]** 나스닥이 {n_chg:.2f}% 불을 뿜고, 국채 금리까지 하락하며 유동성이 숨을 쉬구먼! 세력들이 고삐를 풀었으니 빳빳하게 기세를 타시게."
        elif n_chg < -1.0:
            advice = f"🚨 **[긴급 상황: 정박!]** 나스닥 {n_chg:.2f}% 급락에 비명 소리가 자자하네! 성벽 함락 위기니 무리한 진격은 금물이고 일단 보따리 싸서 소나기를 피하시게."
        elif t_chg > 2.0:
            advice = f"⚠️ **[금리 발작 주의!]** 미 국채 금리가 {t_chg:.2f}%나 치솟으며 시장 목을 죄고 있네! 기술주들 숨통이 막힐 수 있으니 지표 끝단을 매섭게 지켜보시게."
        else:
            advice = f"🧐 **[안개 정국: 관망]** 방향성 없이 눈치싸움이 치열하구먼. 섣불리 낚싯대 던지지 말고 대왕 고래들 움직임이 보일 때까지 자중하며 기다리시게."
