...
if symbol:
    try:
        ...
        if df.empty:
            ...
            # ==================================================================
            # ★ [신호등 분기 논리 - 엄격한 이평선 혼조세 꼬임 방어 우선 적용]
            # 이평선 혼조세(is_ma_tangled)일 때는 5일선 위에 있더라도 20일선 저항 아래에 갇혀 있으면 무조건 관망(노란불)
            # ==================================================================
            try:
            ...
            if is_stop_loss_triggered:
            ...
            elif is_ma_tangled and (p < mid_line or not is_bottom_indicator_ok):
                # ★ [핵심 방어막]: 이평선이 엉킨 혼조세 구간에서 20일선 저항 아래에 있거나 지표가 미흡하면 무조건 관망(노란불) 강제 집행
                ...
                if p < mid_line:
                    final_adv = (
                        f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                        f"<b>[이평선 혼조세 및 20일선 저항]</b> 이동평균선들이 서로 뒤죽박죽 엉켜 있고 머리 위 20일선({mid_line:{fmt_p}}{currency}) 저항 아래에 갇혀 있으니, "
                        "확실한 정배열 분출이 나올 때까지 무리한 추격을 금하고 냉정하게 관망하시게."
                    )
                else:
                    final_adv = (
                        f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                ...
                sig = "🟡 [하락/조정] 음봉 압력 속 추세 이탈 경계"
                col = "#F57C00"
                final_adv = (
                    f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                    "현재 하락 음봉 국면이오니 성벽 및 5일선 이탈에 따른 칼질 관망을 유지하시게."
                )
                ...
                final_adv = (
                    f"• <b>[최종 결론]</b> 보정강도({vol_strength:.1f}점). "
                    "• <b>[진바닥 안착]</b> 지표 충족 및 5일선 안착! 비중 30~50% 수준의 1단계 분할 입절로 접근하시게."
                )
            ...
            if is_band_riding:
                ...
                pullback_action_str = "-> <b>[추세 추종]</b> 상단 밴드 상방 개방! 50% 수확 후 5일선 사수"
            elif is_target_reached:
                pullback_status_str = f"<b>(수학 목표선 저항 도달)</b>"
                pullback_action_str = "-> <b>[50% 수확]</b> 상단 목표 도달 완료"
                ...
                if p < mid_line:
                    pullback_action_str = f"-> <b>[매수 보류]</b> 5일선 회복했으나 <b>머리 위 20일선({mid_line:{fmt_p}}{currency}) 저항 아래</b>에 갇혀 있음"
                else:
