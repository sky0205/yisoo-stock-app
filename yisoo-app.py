...
# 1. 코드 입력창 (여기에 새 종목 코드를 치세요)
t_code = st.text_input("🔢 종목 코드(숫자 6자리) 또는 미장 티커(영문) 입력", value="005930")

# DB 매칭 (없으면 '새 종목'으로 기본값 설정)
info = stock_db.get(t_code, {"name": "새 종목", "market": "국장 (KR)", "target": 0.0})

# 2. 한 줄 설정창 (코드에 따라 실시간으로 값이 변함)
...
with c3: in_target = st.number_input("💎 나의 적정주가", value=float(info["target"]), step=0.1)
...
if st.button("🚀 실시간 정밀 분석 시작"):
    ...
    # 미장/국장 구분하여 데이터 수집
    if "KR" in in_market:
        # 국장은 .KS(거래소) 시도 후 안되면 .KQ(코스닥) 시도
        tech = get_tech_analysis(t_code + ".KS")
        if not tech: tech = get_tech_analysis(t_code + ".KQ")
        price = get_naver_price(t_code)
    else:
        # 미장은 입력한 티커 그대로 사용
        tech = get_tech_analysis(t_code)
    ...
    if price and tech:
        ...
        sum_msg = "에너지가 밴드 상단을 넘보고 있습니다." if price > tech['up'] else "바닥을 확인하며 지지력을 테스트 중입니다."
        ...
        st.table(pd.DataFrame({
            ...
            "상세 진단 (현지수 비교)": [b_diag, "과열(70↑)" if tech['rsi']>70 else "바닥(30↓)" if tech['rsi']<30 else "보통", "단기천장" if tech['wr']>-20 else "단기바닥" if tech['wr']<-80 else "보통", "상승 가속" if tech['macd']>0 else "하락 압력"]
        }))
    else:
        st.error(f"❌ '{t_code}' 데이터를 가져올 수 없습니다. 시장(국장/미장) 선택이 올바른지 확인해 주세요!")
