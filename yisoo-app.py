...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    /* [진짜 핵심 수선] 파란 지붕부터 내용까지 흰색 테두리 하나에 가두는 스타일 */
    .unified-box {
        ...
        margin-bottom: 35px;
        overflow: hidden; /* 제목 배경이 흰색 박스 곡선에 딱 맞게 절단됨 */
        padding: 0 !important;
    }
    .unified-header {
...
def display_global_risk():
    try:
        ...
        advice = "✅ **[미장 쾌청: 진격!]** 나스닥 불 뿜고 금리도 안정세일세!" if n_chg > 0.5 else "🚨 **[긴급 상황: 정박!]** 비명 소리 나니 피신하시게."
    ...
    try:
        ...
        if not df.empty:
            ...
            if v_ratio >= 30 and is_opening:
                ...
                v_adv = f"🚨 **[가짜 상승 주의!]** 주가는 올랐는데 거래량은 빈 수레일세! 속지 마시게." if p_chg > 3 and v_ratio < 100 else f"✅ 세력의 발자국을 추적 중일세."
            ...
            with i2: # RSI
                r_diag = f"● 지수 {rsi_val:.2f}로 **👺 불지옥** 문턱일세! 익절가 빳빳하게 잡으시게." if rsi_val >= 60 else f"● 지수 {rsi_val:.2f}로 **🧊 냉골** 상태일세! 냉정하게 바닥을 보시게." if rsi_val <= 35 else f"● 탐욕과 공포 사이 중립 기어일세."
