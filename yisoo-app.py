...
# 1. 화면 구성 및 할배 캐릭터 스타일 (박스 레이아웃 및 가독성 강화)
...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    /* 박스 스타일링 (글로벌 전황 및 가격선용) */
    .info-card { background-color: #FFFFFF; padding: 20px; border-radius: 15px; border: 3px solid #CFD8DC; box-shadow: 0 4px 6px rgba(0,0,0,0.1); margin-bottom: 15px; }
    .price-box { background-color: #F1F8E9; padding: 15px; border-radius: 10px; border: 2px solid #81C784; text-align: center; }
    ...
    .vol-sub-text { font-size: 24px !important; color: #1565C0 !important; line-height: 1.6; background-color: #FFFFFF; padding: 15px; border-radius: 8px; border-left: 8px solid #1E88E5; }
...
def display_global_risk():
    ...
    with st.container():
        ...
        try:
            ...
            c3.metric("미 국채 10년 (TNX)", f"{tnx_val:.3f}%", f"{tnx_chg:+.2f}%")
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            st.markdown(f"<div class='stock-header'><p style='font-size:30px; color:#1565C0; margin:0;'>{name} ({symbol})</p><p style='font-size:45px; color:#D32F2F; margin:0;'>{format(p, fmt_p)} {currency} <span style='font-size:28px;'>({p_chg:+.2f}%)</span></p></div>", unsafe_allow_html=True)
            
            # 거래량 상세 판독 (유지)
            v_label = "💤 거래침체" if v_ratio < 100 else "📈 거래증가" if v_ratio < 200 else "🔥 거래폭발"
            if v_ratio >= 30 and is_opening:
                v_status = f"🔥 시초 거래폭발 ({v_ratio:.1f}%)"
                v_adv = f"🔥 **[세력 진격!]** 진짜 세력이 미는 거니 빳빳하게 기세 타시게!" if p_chg >= 3 else f"💀 **[비명 포착!]** 성벽 함락 중이니 일단 피신하시게!" if p_chg <= -3 else f"✅ 힘겨루기 중일세."
            else:
                v_status = f"{v_label} ({v_ratio:.1f}%)"
                v_adv = f"🚨 **[가짜 상승 주의!]** 빈 수레일세! 개미 꼬드기는 격이니 속지 마시게." if p_chg > 3 and v_ratio < 100 else f"✅ 세력의 발자국을 추적 중일세."
            ...
            with i1: # Bollinger
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p class='ind-diag'>● {'상단 돌파! 수확하시게.' if p >= up_b else '하단 돌파! 기회!' if p <= low_b else '성벽 사수 보시게.'}</p></div>", unsafe_allow_html=True)
            with i2: # RSI
                st.markdown(f"<div class='ind-box'><p class='ind-title'>RSI (온도)</p><p style='font-size:48px; color:#E65100;'>{rsi_val:.1f}</p><p class='ind-diag'>● {'👺 불지옥' if rsi_val >= 60 else '🧊 냉골' if rsi_val <= 35 else '눈치싸움'} 구간.</p></div>", unsafe_allow_html=True)
            with i3: # Williams
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Williams %R</p><p style='font-size:48px; color:#E65100;'>{will_val:.1f}</p><p class='ind-diag'>● {'🏳️ 개미 항복!' if will_val < -80 else '🧨 천장 광기!' if will_val > -20 else '중간지대'}</p></div>", unsafe_allow_html=True)
            with i4: # MACD
