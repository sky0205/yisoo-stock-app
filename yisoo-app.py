...
st.set_page_config(
    page_title="이수할아버지의 냉정 진단기 v36105", layout="wide"
)
...
st.title("🧐 이수할아버지의 냉정 진단기 v36105 (통신 속도 극대화 튜닝)")
display_global_risk()
st.divider()
    ...
    try:
        ...
        if is_kr:
            ...
            try:
                ...
                res = requests.get(api_url, headers=headers, timeout=2) # 타임아웃 2초 단축
        ...
        st.error(f"👵 아이구! 오류: {e}")import html
from datetime import datetime, timedelta
from zoneinfo import ZoneInfo
from bs4 import BeautifulSoup
import FinanceDataReader as fdr
