...
def display_global_risk():
    ...
    try:
        ...
        if n_chg > 0.5: advice = "✅ **[미장 개장: 진격!]** 기세가 아주 빳빳하구먼! 이 흐름 타시게."
        elif n_chg < -1.0: advice = "🚨 **[긴급: 정박!]** 미장이 피를 흘리며 시작했네! 보따리 싸시게."
        else: advice = "🧐 **[안개 속: 관망]** 시초가 형성 중이니 눈을 부라리고 보시게."
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [수선] 현지 시초장 거래량 및 주가 방향(폭등/폭락) 정밀 판독
            if v_ratio >= 30: # 현지 시간 시초장 거래량 급등 시
                if p_chg >= 3:
                    v_status = "🔥 시초장 주가 폭등 / 거래 폭발"
                    v_adv = f"🔥 **[세력 진격!]** 시초에 거래량이 {v_ratio:.1f}% 터지며 주가가 폭등 중일세! 이건 진짜 세력이 돈 써서 미는 거니 빳빳하게 기세 타시게!"
                elif p_chg <= -3:
                    v_status = "💀 시초장 주가 폭락 / 거래 폭발"
                    v_adv = f"💀 **[비명 포착!]** 시초에 거래량이 {v_ratio:.1f}% 터지며 주가가 폭락 중일세! 성벽이 함락됐으니 무리하게 잡지 말고 일단 피신하시게!"
                else:
                    v_status = "📈 시초장 거래 급등"
                    v_adv = "✅ 거래량은 터졌는데 주가가 갈피를 못 잡고 있구먼. 세력의 힘겨루기 중이니 눈을 부라리고 보시게."
            else:
                v_status = "💤 거래침체" if v_ratio < 100 else "📈 거래증가"
            ...
            # 신호등 및 성벽
            if p >= up_b or rsi_val >= 65: sig, col, adv = "🟢 매도권 진입", "#388E3C", "● 과열권일세! 수익 챙기시게."
            ...
            with i1: # Bollinger
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger</p><p class='ind-status'>{'📈 상승' if p > mid_line else '📉 하락'}</p><p class='ind-diag'>● 하단 터치는 진격 기회, 상단 터치는 익절 기회일세.</p></div>", unsafe_allow_html=True)
            with i2: # RSI
                st.markdown(f"<div class='ind-box'><p class='ind-title'>RSI (온도)</p><p style='font-size:40px; color:#E65100;'>{rsi_val:.2f}</p><p class='ind-diag'>● **{'🧊 냉골' if rsi_val < 35 else '👺 불지옥' if rsi_val > 65 else '미지근'}** 상태일세.</p></div>", unsafe_allow_html=True)
            with i3: # Williams
                w_status = "🏳️ 바닥항복" if will_val < -80 else "🧨 천장광기" if will_val > -20 else "중간지대"
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Williams %R</p><p style='font-size:40px; color:#E65100;'>{will_val:.2f}</p><p class='ind-diag'>● **{w_status}** 구간일세. 냉정하게 보시게.</p></div>", unsafe_allow_html=True)
            with i4: # MACD
                st.markdown(f"<div class='ind-box'><p class='ind-title'>MACD (엔진)</p><p class='ind-status'>{'▲ 정회전' if m_l > s_l else '▼ 역회전'}</p><p class='ind-diag'>● 엔진이 어디로 도는지 보시게.</p></div>", unsafe_allow_html=True)
