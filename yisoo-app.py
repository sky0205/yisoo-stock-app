...
if symbol:
    try:
        ...
        # [68-225] 국장 현주가 오류 해결 및 시초 거래량 보정 통합본
        if is_kr:
            now_local = datetime.now(pytz.timezone('Asia/Seoul'))
            currency = "원"; fmt_p = ",.0f"
            ticker_symbol = f"{symbol}.KS"  # .KS를 붙여야 yfinance가 현주가를 가져오네!
        ...
        if not df.empty:
            # [93-114] 데이터 세척 및 기술 지표 (자네 수식 100% 보존!)
            df = df.ffill().dropna()
            ...
            # RSI, Williams %R, MACD, Bollinger (엔진실 점검 완료!)
            delta = df['Close'].diff()
            ...
            # --- [핵심] 거래량 및 미장식 시간 보정 로직 (자네 지시사항!) ---
            import datetime as dt
            now_time = dt.datetime.now(now_local.tzinfo)
            ...
            # 미장식 시간 보정 (장 시작 후 경과 시간 기준)
            s_h, s_m = (9, 0) if is_kr else (9, 30)
            elapsed = (now_time.hour - s_h) * 60 + (now_time.minute - s_m)
            if now_time.weekday() >= 5 or elapsed > 390: elapsed = 390
            elif elapsed < 10: elapsed = 10
            
            # 국장도 시초 기세를 읽는 vol_strength 완성!
            vol_strength = (v_ratio / (elapsed / 390))

            # [전광판 출력] 자네 양식 그대로!
            st.markdown("### 📊 현재주가현황")
            display_price = f"{p:,.0f}{currency} (전일비: {p_diff:+,.0f} / {p_chg:+.2f}%)" if is_kr else f"{currency}{p:,.2f} (전일비: {p_diff:+.2f} / {p_chg:+.2f}%)"
            ...
            # 거래량 상세 판독 및 호통 (자네 기존 문구 완벽 복구)
            v_label = "💤 거래침체" if v_ratio < 100 else "📈 거래증가" if v_ratio < 200 else "🔥 거래폭발"
            if v_ratio >= 30 and is_opening:
                v_status, v_adv = f"📈 시초 거래기세 ({vol_strength:.0f}점)", f"✅ 현재 시간 기세 대비 거래강도 {vol_strength:.0f}점으로 세력의 발자국을 추적 중일세."
            else:
            ...
            # [신호등 및 가격 카드] 자네 양식 그대로!
            if p >= up_b or rsi_val >= 60: sig, col, s_adv = "🟢 매도권 진입", "#388E3C", f"● {'👺 불지옥!' if rsi_val >= 60 else '수익 챙기시게.'}"
            elif p <= (low_b * 1.005) or rsi_val <= 35: sig, col, s_adv = "🔴 매수권 진입", "#D32F2F", "● 🧊 바닥권일세."
            else: sig, col, s_adv = "🟡 관망 및 대기", "#FBC02D", "● 눈치싸움 중."
            ...
            if p >= up_b or rsi_val >= 60:
                ...
                if vol_strength > 150: final_adv = f"🚨 **[최종 결론]** 거래량({vol_strength:.0f}점) 실린 폭락세일세! **무조건 관망하시게!**"
                else: final_adv = f"🧐 **[최종 결론]** 거래강도({vol_strength:.0f}점). 성벽 위태롭네. **관망하시게!**"
            elif p <= (defense_line * 1.01):
                if vol_strength > 150: final_adv = f"🔥 **[최종 결론]** 거래량({vol_strength:.0f}점) 실린 진짜 바닥권일세! **강력 매수** 시점이네!"
                else: final_adv = f"🛡️ **[최종 결론]** 거래강도({vol_strength:.0f}점). 바닥권이나 기세 약하네. **분할 매수**하시게!"
            elif rsi_val <= 35:
                final_adv = f"💎 **[최종 결론]** 거래강도({vol_strength:.0f}점). 지표 온도가 냉골일세. **정찰병 파견**하시게!"
            else:
                final_adv = f"📈 **[최종 결론]** 거래강도({vol_strength:.0f}점). 추세 살아있구먼. **보유(홀딩)**하시게!"
