...
def display_global_risk():
    ...
    try:
        ...
        if n_chg < -0.5:
            advice = "🚨 [미장 소나기] 지수는 폭락하고 금리는 요동치네! 보따리 꽁꽁 묶고 정박하시게!"
        elif n_chg > 0.5:
...
# [제목] 어르신의 빳빳한 캐릭터 복구
st.title("🛡️ 이수할아버지의 냉정 진단기 v36056")
display_global_risk() 
st.divider()
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # 네 기둥 지수 상세 진단 (Bollinger/RSI/Williams/MACD) 부활
            i1, i2, i3, i4 = st.columns(4)
            with i1: st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p class='ind-status'>{'📈 상승' if p > mid_line else '📉 하락'}</p><p class='ind-diag'>중앙선({format(mid_line, fmt)}) 기준 기세 판독.</p></div>", unsafe_allow_html=True)
            with i2: st.markdown(f"<div class='ind-box'><p class='ind-title'>RSI (온도)</p><p class='ind-status' style='color:#E65100;'>{rsi_val:.2f}</p><p class='ind-diag'>탐욕과 공포의 줄다리기.</p></div>", unsafe_allow_html=True)
            with i3: st.markdown(f"<div class='ind-box'><p class='ind-title'>Williams %R</p><p class='ind-status'>진단중</p><p class='ind-diag'>과매수/과매도 변곡점 판독.</p></div>", unsafe_allow_html=True)
            with i4: st.markdown(f"<div class='ind-box'><p class='ind-title'>MACD (엔진)</p><p class='ind-status'>판독중</p><p class='ind-diag'>추세의 엔진 강도 진단.</p></div>", unsafe_allow_html=True)
