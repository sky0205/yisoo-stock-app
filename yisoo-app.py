...
# 1. 화면 구성 및 할배 캐릭터 스타일 (제목-수치 완전 봉인 레이아웃)
...
st.markdown("""
    ...
    * { font-weight: bold !important; font-family: 'Nanum Gothic', sans-serif; color: #263238; }
    ...
    /* [어르신 특명] 제목과 내용을 완전히 하나로 가두는 흰색 통합 박스 */
    .unified-card {
        ...
        margin-bottom: 35px;
        padding: 0 !important; /* 박스 자체의 패딩을 없애야 제목이 테두리에 딱 붙음 */
        overflow: hidden; /* 제목 배경이 박스 곡선에 맞게 절단됨 */
    }
...
# 글로벌 전황: 제목 파란 지붕부터 내용까지 "진짜 한 상자"에 가둠
def display_global_risk():
    try:
        ...
        # 흰색 박스(`unified-card`)를 먼저 열고, 그 안에 파란 제목(`unified-header`)을 넣음
        st.markdown(f"""
        ...
        st.markdown("</div></div>", unsafe_allow_html=True) # 보따리 닫기
    ...
    try:
        ...
        if not df.empty:
            ...
            if v_ratio >= 30 and is_opening:
                ...
                v_adv = f"🔥 **[세력 진격!]** 거래량이 빳빳하게 터지며 폭등 중일세! 진짜 세력이 미는 거니 기세 타시게!" if p_chg >= 3 else f"💀 **[비명 포착!]** 거래량이 {v_ratio:.1f}% 터지며 폭락 중일세!"
            else:
            ...
            with i1: # Bollinger
                bb_diag = f"● **[비상: 상단 돌파!]** 하늘 찌르는구먼! 탐욕 버리고 익절 준비 하시게." if p >= up_b else f"● **[비상: 하단 돌파!]** 바닥권일세! 진짜 진격 기회일세." if p <= low_b else f"● 성벽 사수 확인 전까지는 기다리시게."
