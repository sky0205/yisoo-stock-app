...
# 1. 화면 구성 (어르신 원본 스타일 및 문구 100% 보존)
...
# 1. 미장 분석 내용 상세 복구 (image_abc7b5.png 기반)
def display_global_risk():
    try:
        ...
        if n_chg > 0.5: advice = f"✅ **[미장 쾌청: 진격!]** 나스닥 {n_chg:.2f}% 불을 뿜으며 유동성이 숨을 쉬구먼! 세력들이 고삐 풀었으니 빳빳하게 기세를 타시게."
        elif n_chg < -1.0: advice = f"🚨 **[긴급 상황: 정박!]** 나스닥 {n_chg:.2f}% 급락에 비명 소리가 자자하네! 성벽 함락 위기니 무리한 진격은 금물이고 일단 보따리 싸서 소나기를 피하시게."
        else: advice = "🧐 **[안개 정국: 관망]** 방향성 없이 눈치싸움이 치열하구먼. 대왕 고래들 움직임 보일 때까지 자중하며 기다리시게."
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # 3. 실전 필살 조언 로직 수선 (image_ab5a41.png 반영)
            ...
            else: adv_text = "🎣 낚싯대만 던져두고 지표 바닥권을 기다리십시오."
            ...
            # 4. 지수 상세 설명 원본 100% 복구 (image_abbc54.png 기반)
            st.divider()
            ...
            with i1: # Bollinger
                ...
                bb_diag = f"● 현재 **중앙선 위**에서 기세를 타는 중입니다. 상단 성벽 돌파 시도는 긍정적이나, 실패 시 중앙선까지 밀릴 수 있으니 주의하십시오." if bb_pos=="⚖️ 눈치보기(상)" else f"● 현재 **중앙선 아래**에서 빌빌대고 있습니다. 하단 성벽까지 밀릴 위험이 크니 섣부른 진격은 금물입니다." if bb_pos=="⚖️ 눈치보기(하)" else f"● 성벽 밖 **과열 권역**입니다. 평균 회귀 본능에 의해 수익 실현 소나기가 올 수 있으니 보따리 쌀 준비 하십시오." if bb_pos=="🔥 폭주" else f"● 하단 성벽이 무너진 **공포 구간**입니다. 투매 비명소리가 잦아들 때까지 절대 칼날을 잡지 마십시오."
                st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p style='color:{bb_color}; font-size:32px;'>{bb_pos}</p><p class='ind-diag'>{bb_diag}</p></div>", unsafe_allow_html=True)
            with i2: # RSI
                r_stat = "👺 불지옥" if rsi_val > 65 else "🧊 냉골" if rsi_val < 35 else "미지근"
                r_diag = f"● 온도 {rsi_val:.2f}로 **탐욕의 불지옥** 직전입니다. 남들이 환호할 때 냉정하게 분할 매도로 수익을 확정 지으십시오." if r_stat=="👺 불지옥" else f"● 온도 {rsi_val:.2f}의 **냉골 상태**입니다. 지수가 30 근처까지 완전히 식어버릴 때까지 기다리십시오." if r_stat=="🧊 냉골" else f"● 매수와 매도가 팽팽한 **줄다리기** 중입니다. 방향성 없는 싸움에 끼어들어 수수료만 버리지 마십시오."
            ...
            with i3: # Williams
                ...
                w_diag = f"● **불나방 광기(-20↑)** 직전입니다! 지수 {will_val:.2f}는 천장 근처이니 추격하지 말고 후퇴를 준비하십시오." if will_val > -35 else f"● **항복 지점(-75↓)** 근처입니다. 지수 {will_val:.2f}로 바닥권이니 이제 분할 매수 보따리를 푸십시오." if will_val < -65 else f"● 현재 지수 {will_val:.2f}로 **안개 속**입니다. 방향이 정해질 때까지 자중하며 째려만 보십시오."
            ...
            with i4: # MACD
                ...
                macd_diag = f"● **상승 엔진**이 힘차게 돌고 있으나, 상단 저항을 이겨내는지 확인하며 진격 수위를 조절하십시오." if m_l > s_l else f"● **하강 엔진**으로 역회전 중입니다. 섣부른 진격보다는 정박하여 폭풍우가 지나가길 기다리는 게 상책입니다."
                st.markdown(f"<div class='ind-box'><p class='ind-title'>MACD (엔진)</p><p style='font-size:32px; color:#D32F2F;'>{macd_status}</p><p class='ind-diag'>{macd_diag}</p></div>", unsafe_allow_html=True)
