...
def get_global_full(ticker, name, comment_up, comment_down):
    try:
        ...
        return f"""<div class='global-box'><div class='global-title'>{name}</div>
                   ...
                   <div class='global-diag'><b>👴 냉정 평가:</b> {diag}</div></div>"""
    except: return f"<div class='global-box'>{name}<br>장부 확인 불가</div>"

g1.markdown(get_global_full("^IXIC", "나스닥 (NASDAQ)", 
    "기술주들이 억지로 버티나 거품이 여전하네. 가짜 반등일세.", "금리 압박에 숨이 턱턱 막히는 형국이야. 조심들 하셔."), unsafe_allow_html=True)
g2.markdown(get_global_full("^GSPC", "S&P 500", 
    "큰 집들이 체면치레 중이나 안개는 안 걷혔어.", "시장 전체가 도살장 분위기로 변하고 있구먼. 몸 사리시게."), unsafe_allow_html=True)
g3.markdown(get_global_full("^TNX", "미국채 10년 금리", 
    "이놈이 튀어 오르면 주식은 곡소리 나는 법이야.", "잠시 숨을 고르나 언제든 비수가 될 놈일세. 부라리고 보셔."), unsafe_allow_html=True)
    ...
    try:
        ...
        if not df.empty:
            ...
            st.markdown(f"""
                ...
                <p style='font-size:45px; color:{color_p}; margin:0;'>
                {format(p, fmt)} {currency} 
                <span style='font-size:28px;'> (전일비: {format(diff, diff_fmt)})</span>
                </p>
                </div>
                """, unsafe_allow_html=True)
            ...
            with i1: # BB
                st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ Bollinger Bands</p><span class='ind-value'>{format(mid_line, fmt)}</span><p class='ind-diag'>● 가격이 중앙선 {'위에서 노니 기세는 있으나' if p > mid_line else '아래로 꺾여 기운이 없구먼.'} 상단 성벽({format(up_b, fmt)}) 돌파 여부를 부라리고 보게나.</p></div>", unsafe_allow_html=True)
            with i2: # RSI
            ...
            with i4: # MACD
                st.markdown(f"<div class='ind-box'><p class='ind-title'>🛡️ MACD (추세엔진)</p><span class='ind-value' style='font-size:40px !important;'>{'▲ 상승' if m_l > s_l else '▼ 하락'}</span><p class='ind-diag'>● 엔진이 {'정방향으로 도는 중이나' if m_l > s_l else '역회전 중이야.'} 추세 마디가 바뀌는 맥점을 독하게 째려보게나.</p></div>", unsafe_allow_html=True)

    except Exception as e: st.error(f"장부를 불러올 수 없구먼 (오류: {e})")

st.write("---")
st.caption("분석가 서강윤: 2026년 실시간 장부 및 v36056 최종 양식(전일비+미장평가) 적용")
