...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            # [핵심 수선] 시초 거래량 판독 로직 (오전 11시 전 30% 돌파 시)
            ...
            with i1: # Bollinger
                bb_diag = f"● 하단 성벽 아래일세! 지금은 지지 확인 후 진격 기회를 볼 때구먼!" if p <= low_b else f"● 지지 확인 전까지 낚싯대만 던지시게."
            ...
            with i3: # Williams
                ...
                w_diag = f"● 지수 {will_val:.2f}로 **{w_status}** 구간일세! 바닥 끝단이니 여기서 고개 들면 무조건 진격일세!" if will_val < -80 else f"● 현재 안개 속일세."
