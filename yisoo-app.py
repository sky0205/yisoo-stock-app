    ...
    try:
        ...
        if df.empty:
            ...
            if final_code == "BREAK_MA20_CONFIRMED":
                ...
                # 무조건 현재 실시간 3대 지표 상태를 직관적으로 따져서 점수 확정
                _sync_score = int((1 if df["Close"].iloc[-1] <= (low_b.iloc[-1] * 1.02) else 0) + (1 if rsi_series.iloc[-1] <= 40 else 0) + (1 if will_series.iloc[-1] <= -75 else 0))
