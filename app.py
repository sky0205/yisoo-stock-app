...
# 1. 화면 설정 및 종목 기억장치
st.set_page_config(page_title="이수 Stock Analyzer v100", layout="wide")

# 오늘 본 종목들을 [코드: 이름] 쌍으로 저장하는 바구니
if 'stock_memory' not in st.session_state:
    st.session_state.stock_memory = {"005930.KS": "삼성전자", "IONQ": "아이온큐", "NVDA": "엔비디아"}
...
# 2. 메인 화면: 번호 입력창
st.title("👨‍💻 이수할아버지의 주식분석기 v100")
st.write("---")

st.subheader("🔍 종목 번호(6자리)나 티커를 입력하세요")
u_input = st.text_input("번호 입력 후 엔터 (예: 000660)", key="main_search")

# [즉시 처리] 입력 즉시 리스트에 추가하고 화면 전환
if u_input:
    code = u_input.upper().strip()
    if code.isdigit() and len(code) == 6:
        full_code = code + ".KS" # 한국 주식 우선
    else:
        full_code = code
    
    # 에러 방지용 이름 가져오기 (매우 빠른 방식)
    try:
        t = yf.Ticker(full_code)
        s_name = t.fast_info.get('last_price', None) # 데이터 존재 확인용
        if s_name:
            st.session_state.stock_memory[full_code] = full_code # 우선 코드로 저장
            st.session_state.now_ticker = full_code
            st.rerun()
    except:
...
sel_ticker = st.selectbox("📋 오늘 분석 중인 종목 리스트", options=options, 
                          index=options.index(st.session_state.now_ticker) if st.session_state.now_ticker in options else 0)
st.session_state.now_ticker = sel_ticker

# 4. 데이터 엔진 (가장 튼튼한 방식)
@st.cache_data(ttl=60)
def load_data_v100(ticker):
    try:
...
if sel_ticker:
    ...
    if df is not None and not df.empty:
        ...
        st.error("데이터를 가져올 수 없습니다. 번호를 다시 확인해 보세요.")
