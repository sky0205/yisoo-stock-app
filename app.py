...
# 1. 화면 설정 및 종목 기억장치
st.set_page_config(page_title="이수 Stock Analyzer v89", layout="wide")

# 선생님이 오늘 검색하고 분석할 종목들을 담아두는 바구니입니다.
...
# 2. [v20 스타일] 사이드바 종목 검색 및 등록란
with st.sidebar:
    st.markdown("### 🔎 종목 검색 및 추가")
    st.write("새로운 종목을 리스트에 넣으세요.")
    
    # 입력칸을 더 큼직하고 명확하게 배치
    new_stock_name = st.text_input("1. 종목 이름 입력", placeholder="예: 테슬라", key="name_input")
    new_stock_code = st.text_input("2. 티커/코드 입력", placeholder="예: TSLA", key="code_input")
    
    if st.button("✨ 분석 리스트에 등록", use_container_width=True):
        if new_stock_name and new_stock_code:
            st.session_state.stock_list[new_stock_name] = new_stock_code
            st.success(f"'{new_stock_name}' 등록 완료!")
            st.rerun()
    
    st.write("---")
    st.markdown("### 📋 현재 분석 리스트")
    # 등록된 종목들을 한눈에 확인
    for name, code in st.session_state.stock_list.items():
...
# 4. 메인 화면 구성
st.title("👨‍💻 이수할아버지의 주식분석기 v89")
st.write("---")

# 종목 선택Dropdown (가장 위에서 선택)
...
if code:
    ...
    if df is not None and not df.empty:
        ...
        if rsi <= 35 and not macd_up:
            memo_content += "💡 **알림**: 가격은 바닥이지만(RSI), 아직 기세는 하락(MACD) 중입니다. <b>'분할 매수'</b>가 안전합니다.<br>"
        elif rsi <= 35 and macd_up:
            memo_content += "✅ **추천**: 바닥 확인 후 기세가 살아났습니다. <b>'적극 매수'</b> 가능 구간입니다.<br>"
        
        if ma20_up: memo_content += "✅ **20일선**: 주가가 빨간 중간선을 <b>상향 돌파</b>했습니다. 매수하기 좋은 시점입니다.<br>"
        else: memo_content += "❌ **20일선**: 아직 중간선 아래에 있습니다. 반등을 더 기다리세요.<br>"
        
        if macd_up: memo_content += "✅ **기세**: 파란선(MACD)이 주황선 위에 있으니 <b>보유</b> 관점을 유지하세요.<br>"
        else: memo_content += "❌ **기세**: 파란선이 아래에 있으니 서두르지 마세요.<br>"
        ...
        # [4] 볼린저 밴드 그래프
        st.write("---")
        st.write("### 📊 주가 흐름 및 볼린저 밴드 (20일선 돌파 확인)")
