...
# 1. Page Config (Must stay in English)
...
def get_safe_data(ticker):
    try:
        ...
        # Handle Multi-Index Columns (The main cause of blank metrics)
...
if t_ticker:
    df = get_safe_data(t_ticker)
    if (df is None or df.empty) and ".KS" in t_ticker:
        df = get_safe_data(t_ticker.replace(".KS", ".KQ"))
    ...
    if df is not None and 'close' in df.columns:
        ...
        if curr_p >= y_high * 0.97:
            st.markdown(f"<div class='info-box'>🚀 <strong>신고가 영역:</strong> 돌파 기세가 강합니다!</div>", unsafe_allow_html=True)

        m1, m2, m3, m4 = st.columns(4)
        m1.metric("현재가", f"{curr_p:,.2f}")
        m2.metric("RSI (과열도)", f"{rsi_val.iloc[-1]:.1f}")
        m3.metric("윌리엄 %R", f"{w_r.iloc[-1]:.1f}")
        m4.metric("1년 최고가", f"{y_high:,.2f}")
        ...
        st.error("데이터를 가져오는 데 실패했습니다. 영문 원본으로 실행해 주세요.")
