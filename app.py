...
# 1. 화면 설정 및 제목 확인 (v148)
st.set_page_config(page_title="이수 주식앱 v148", layout="wide")
...
st.markdown("""
    ...
    .memo-box { padding: 20px; border-radius: 12px; background-color: #E0F2FE; border-left: 10px solid #0EA5E9; font-size: 18px; font-weight: bold; line-height: 1.6; }
    </style>
    """, unsafe_allow_html=True)

# 2. 메인 화면
st.title("👨‍💻 이수할아버지의 주식분석기 v148")
st.success("✅ 이제 최신 엔진으로 가동 중입니다! v106은 잊어주세요.")
    ...
    try:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                return df, "해외 서버(Yahoo) 우회 성공"
        except: return None, "데이터 통로 차단됨"
    return None, "데이터 없음"

if ticker:
    df, msg = fetch_data_v148(ticker)
    if isinstance(df, pd.DataFrame):
        ...
        st.markdown(f"<div class='memo-box'>🚩 **분석 정보**: {msg}<br>현재 주가가 20일선 {'위' if close.iloc[-1] > ma20.iloc[-1] else '아래'}에 있습니다.</div>", unsafe_allow_html=True)
